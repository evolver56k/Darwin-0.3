	...
	thread_t	*child_thread)		/* OUT */
{
	...
	*new_thread = thread_template;
	...
	uarea_zero(new_thread->_uthread);
...
void reaper_thread(void)
{
	...
	/*
	 * make reaper a high priority thread
	 * with fix priority scheduling policy
	 */
	th->priority = BASEPRI_SYSTEM + 3; 
	th->sched_pri = BASEPRI_SYSTEM + 3;
	th->policy = POLICY_FIXEDPRI;
	th->vm_privilege = TRUE;
	th->sched_data = min_quantum * 4;

	/* Dedicate a stack for the reaper */
	stack_privilege(th);
