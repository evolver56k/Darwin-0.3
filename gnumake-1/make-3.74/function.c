	...
	if (pid < 0)
	...
	else
	  {
	   ...
	   char *buffer;
           ...
           tempdir = getenv("TEMP");
           if (!tempdir) fatal("Please set the TEMP environment variable"); /* Better than a bus error in strcpy */
           strcpy (tmp_output, tempdir);
           strcat (tmp_output, "\\shXXXXXX");
	   mktemp (tmp_output);
	   child_stdout = open (tmp_output,
				O_WRONLY|O_CREAT|O_TRUNC|O_TEXT, 0644);
           save_stdout = dup (1);
	   dup2 (child_stdout, 1);
	   clean_spawnvp (P_WAIT, argv[0], argv);
	   dup2 (save_stdout, 1);
	   close (child_stdout);
	   close (save_stdout);
