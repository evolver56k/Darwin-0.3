...
#ifdef WIN32
struct file *
lookup_file_for_nt (name)
     char *name;
{
    struct file *f = lookup_file (name);
    /* FOO.EXE is also a target if FOO is mentioned in the makefile.
      This is done to support using the same makefile on UNIX and NT,
      even though executables have a suffix under NT */
    if (!f)
      {
      char* s = strrchr (name, '.');
      if (s && !stricmp (s, ".exe"))
