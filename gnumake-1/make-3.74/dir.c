...
#if defined (WIN32)
    /* 
    ** NT does not support inodes, so we can't use them
    ** to guarantee uniqueness.  Use the path instead.
    */
    const char* path;		/* path to this dir */
#else
  ...
  if (dir == 0)
      ...
      else
	{
	  ...
	  struct directory_contents *dc;
          ...
          {
          char* s;
          for (hash=0, s=name; *s; s++) hash = hash ^ *s;
          }
#else
...
#if defined (WIN32)
              /* Allocate  a copy of the path.  I would like to */
              /* free  this copy when  the  structure is freed, */
              /* but it appears that these structures are never */
              /* freed!                                         */
              dc->path = savestring (name, strlen (name));
#else
	      dc->dev = st.st_dev;
	      dc->ino = st.st_ino;
#endif
...
#ifdef WIN32
      /* Note that directories are read lazily, and so the directory is held
         open until it is closed by dir_contents_file_exists_p(), and this
         only happens if the directory is searched for a nonexistent file.
         Under Windows NT, you cannot delete a directory if a process has it
         open, so "make clean" can sometimes fail because make itself has
         some VPATHed directory open.  The solution is to always force a
         nonlazy read by querying for a nonexistent file before returning
         the new directory.  Since slashes are not legal filename
         characters, the file a/\b is guaranteed not to exist.  */
      dir_contents_file_exists_p (dir->contents, "a/\\b");
#endif
...
#if defined (WIN32)
          printf ("# %s (path %s): could not be opened.\n",
                  dir->name, dir->contents->path);
#else
