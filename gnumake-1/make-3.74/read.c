...
#if NeXT || NeXT_PDO
      else if ((next_flag & NEXT_FINDFILE_FLAG) && word1eq ("findfile", 8))
        {
          /* A `findfile' has been found.
             Handle it as above, but don't choke if the file
             doesn't really exist.  My quick hack is to treat
             it just like a Makefile from the MAKEFILES
             environment variable. */
          struct conditionals *save = conditionals;
          struct conditionals new_conditionals;
          p = allocated_variable_expand (next_token (p + 8));
          if (*p == '\0') {
              makefile_error (filename, lineno, "no filename for `findfile'");
              continue;
	  }
          p2 = end_of_token (p);
          if (*p2 != '\0') {
              *p2++ = '\0';
              if (*next_token (p2) != '\0')
                makefile_error(filename, lineno,
                               "extraneous text after `findfile'");
	  }
	  if (*p == '/') {
	      makefile_error (filename, lineno, 
			      "filename for `findfile' begins with `/'");
	      free(p);
	      continue;
	  }
	  {
	      ...
	      while (1) {
		  name = concat (prefix, p, "");
		  if (stat(name, &s) >= 0 && (s.st_mode & S_IFMT) == S_IFREG)
		      break;
		  free(name);
		  name = concat("../", prefix, "");
		  free(prefix);
	      ...
	      free(prefix);
	      if (name == 0) {
		  makefile_error(filename, lineno, "can't find `%s' ", p);
		  free(p);
		  continue;
	      }
	      free(p);
...
#undef	word1eq
      ...
      else
	{
	  ...
	  p = index (p2, ':');
          while (p != 0)
            {
            /* Skip over a ':' escaped by a backslash */
            if (p[-1] == '\\')
              {
              register char *q = &p[-1];
            ...
            /* Skip over a ':' drive specifier */
            /* I should probably ensure that p[-2] is blank, but */
            /* I don't want to risk a seg fault.                 */
            else if (p != 0 && (p[1] == '/' || p[1] == '\\') && isalpha (p[-1]))
              {
              p = index (p+1, ':');
              }
#endif
            /* If neither of the above, stop skipping */
            else
              {
              break;
              }
