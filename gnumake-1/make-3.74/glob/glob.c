  ...
  for (i = 0; i < n; ++i)
    {
      ...
      /* note that eltlen needs to be one larger if MARK is set,
         but we have no way of knowing this.  Allocate space for
	 the trailing slash in all cases just to be safe. */
      char *new = (char *) malloc (dirlen + 1 + eltlen + 1);
      if (new == NULL)
	{
	  while (i > 0)
