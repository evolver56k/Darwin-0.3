...
lu_getpwuid(int uid)
{
	...
	xdrmem_create(&xdr, lookup_buf, datalen * BYTES_PER_XDR_UNIT, XDR_DECODE);
	lu_pw = NULL;
...
lu_endpwent(void)
{
	...
	{
		freeold();
		vm_deallocate(task_self(), (vm_address_t)pw_data, pw_datalen * BYTES_PER_XDR_UNIT);
		pw_data = NULL;
	}
}
...
lu_getpwent()
{
	...
	if (pw_start == 1)
	{
		...
		xdrmem_create(&pw_xdr, pw_data, pw_datalen * BYTES_PER_XDR_UNIT,
			XDR_DECODE);
