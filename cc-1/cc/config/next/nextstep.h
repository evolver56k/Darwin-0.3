...
#if defined(NeXT) || !defined(CROSS_COMPILE)
...
"%{!fsyntax-only: \
   %{!c:%{!M:%{!MM:%{!E:%{!precomp:%{!S: \
      %{dynamiclib:/bin/libtool %{arch*:-arch_only %T} %{!all_load:-noall_load}\
	%{current_version:}%{compatibility_version:} %J} \
     %{!dynamiclib:%{.C:/bin/ld++}%{.M:/bin/ld++}%{.cc:/bin/ld++} \
		  %{!.C:%{!.M:%{!.cc:%{ObjC++:/bin/ld++}%{!ObjC++:/bin/ld}}}} \
		  -arch %T %{@:-arch_multiple}} \
			%l %X %{force_cpusubtype_ALL}\
			%{@:-o %f%g-%T.out}%{!@:%{o}} \
...
"%{!fsyntax-only: \
   %{!c:%{!M:%{!MM:%{!E:%{!precomp:%{!S: \
      %{dynamiclib:/usr/bin/libtool %{arch*:-arch_only %T} \
		%{!all_load:-noall_load} \
		%{current_version:}%{compatibility_version:} %J} \
...
#warning ****** Disabled objcunique *******
/* Darwin doesn't have a working objcunique */
#if 0
"
		%{!.C:%{!.M:%{!.cc:%{ObjC++:\\| /usr/bin/c++filt}}}}\
     %{!dynamiclib:\n/usr/bin/objcunique %{prebind} %{noprebind} \
		   %{F*} %{L*} %{@:%f%g-%T.out} %{!@:%{o*:%*}}}}}}}}}}"
#endif
#endif
