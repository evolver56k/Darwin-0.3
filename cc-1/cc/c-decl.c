...
extern void add_vararg_func(char *, char);
#endif
...
#ifdef NeXT
#ifdef HPPA
	/* record that this is a vararg/stdarg function which must 
	   be taken special care of when it is called. */

	{
	  tree last = tree_last (TYPE_ARG_TYPES (type));
	  
	  if (last && (TYPE_MAIN_VARIANT (TREE_VALUE (last)) 
		       != void_type_node))
	    add_vararg_func(IDENTIFIER_POINTER(declarator), '1');
	  else
	    add_vararg_func(IDENTIFIER_POINTER(declarator), '0');
	}
#endif
#endif
