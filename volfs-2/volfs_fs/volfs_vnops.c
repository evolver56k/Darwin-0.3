        ...
        struct ucred *a_cred; struct proc *a_p; } */ *ap;
{
    ...
    a_vap->va_atime.tv_sec = 0;
    a_vap->va_atime.tv_nsec = 0;

    a_vap->va_mtime.tv_sec = 0;
    a_vap->va_mtime.tv_nsec = 0;

    a_vap->va_ctime.tv_sec = 0;
    ...
    struct vnode      **ret_vnode;
{
    ...
    for (mp = mountlist.cqh_first; mp != (void *)&mountlist; mp = mp->mnt_list.cqe_next)
      {
        //DBG_VOP(("\tcur_mount: 0x%x, id: %d\n", mp, mp->mnt_stat.f_fsid.val[0]));
        if (mp->mnt_stat.f_fsid.val[0] == id)
          {
            cur_mount = mp;
            break;
          }
...
search_vnodelist:
    ...
    if (cur_vnode) {
        ...
        if (vget(cur_vnode, LK_EXCLUSIVE | LK_INTERLOCK, current_proc()) != 0) {
    ...
    else
      {
        MALLOC(cur_privdata, struct volfs_vndata *,
               sizeof(struct volfs_vndata), M_VOLFS, M_WAITOK);
        retval = getnewvnode(VT_VOLFS, our_mount, volfs_vnodeop_p, &cur_vnode);
        if (retval != 0) {
            FREE(cur_privdata, M_VOLFS);
            return retval;
        };
 ...
 */
static int
get_filevnode(parent, id, ret_vnode)
    struct vnode      	*parent;
    u_int 				id;
    struct vnode     	**ret_vnode;
{
    ...
    priv_data = (struct volfs_vndata *) parent->v_data;
	/*
	 * Special case 2 to mean the root of a file system
	 */
	if (id == 2)
		retval = VFS_ROOT(priv_data->fs_mount, ret_vnode);
	else
    	retval = VFS_VGET(priv_data->fs_mount, &id, ret_vnode);
        ...
        **a_vpp; struct componentname *a_cnp; } */ *ap;
{
	...
	else if (cnp[0] == '@')
	{
		if ((namelen == 1) && (priv_data->vnode_type != VOLFS_ROOT)) {
			ret_err = VFS_ROOT(priv_data->fs_mount, ap->a_vpp);
        } else {
	...
	else if (namelen <= 10 && cnp[0] > '0' && cnp[0] <= '9') /* 10 digits max lead digit must be 1 - 9 */
	{
		 ...
		 * the terminating null or there was other crap in the
		 * number
		 */
		if ((check_ptr - cnp) == namelen)
		{
		    if (priv_data->vnode_type == VOLFS_ROOT)
				...
				ret_err = get_filevnode(ap->a_dvp, id, ap->a_vpp);
		}
