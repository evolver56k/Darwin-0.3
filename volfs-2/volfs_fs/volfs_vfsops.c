...
//      call_kdp();
    ...
    if (newvfsconf) {
         ...
         */
        MALLOC(*opv_desc_vector_p, PFI *, vfs_opv_numops*sizeof(PFI), M_VOLFS, M_WAITOK);
	...
	struct proc *p;
{
    ...
    MALLOC(priv_vn_data, struct volfs_vndata *, sizeof(struct volfs_vndata), M_VOLFS, M_WAITOK);
    error = getnewvnode(VT_VOLFS, mp, volfs_vnodeop_p, &root_vp);
    if (error != 0)
      {
        FREE(priv_mnt_data, M_VOLFS);
        FREE(priv_vn_data, M_VOLFS);
	...
	struct proc *p;
{
	...
	sbp->f_bsize = 512;
	sbp->f_iosize = 512;
	sbp->f_blocks = 1024;	// lies, damn lies and virtual file systems
