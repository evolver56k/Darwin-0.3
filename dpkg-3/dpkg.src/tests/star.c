...
char *thisname = "star";
char *printforhelp = "printforhelp";

static int Read (void *userData, char *buffer, int length)
{
...
static int ExtractFile (TarInfo * i)
{
...
static int MakeDirectory (TarInfo * i)
{
...
static int MakeHardLink (TarInfo * i)
{
  printf (_("Hard Link: %s\n"), i->Name);
...
static int MakeSymbolicLink (TarInfo * i)
{
...
static int MakeSpecialFile (TarInfo * i)
{
  printf (_("Special File: %s\n"), i->Name);
...
int main (int argc, char **argv)
{
  ...
  if (status != 0) {
    fflush (stdout);
    fprintf (stderr, _("Error in archive format.\n"));
    return -1;
  } else {
    return status;
  }
