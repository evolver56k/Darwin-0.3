...
int packagelist::describemany(char buf[], const char *prioritystring,
  ...
  case sso_avail:
    ...
    ssostring= gettext(ssastrings[pps->ssavail]);
    ssoabbrev= gettext(ssaabbrevs[pps->ssavail]);
    statindent++;
    break;
  case sso_state:
    if (pps->ssstate == -1) break;
    ssostring= gettext(sssstrings[pps->ssstate]);
    ssoabbrev= gettext(sssabbrevs[pps->ssstate]);
    statindent++;
    break;
  case sso_unsorted:
    break;
  ...
  if (!prioritystring) {
    if (!section) {
      strcpy(buf, ssostring ? ssostring : _("All packages"));
      return statindent;
    } else {
      if (!*section) {
        sprintf(buf,_("%s packages without a section"),ssoabbrev);
      } else {
        sprintf(buf,_("%s packages in section %s"),ssoabbrev,section);
      }
      return statindent+1;
    }
  } else {
    if (!section) {
      sprintf(buf,_("%s %s packages"),ssoabbrev,prioritystring);
      return statindent+1;
    } else {
      if (!*section) {
        sprintf(buf,_("%s %s packages without a section"),ssoabbrev,prioritystring);
      } else {
        sprintf(buf,_("%s %s packages in section %s"),ssoabbrev,prioritystring,section);
      }
      return statindent+2;
    }
  }
...
void packagelist::redraw1itemsel(int index, int selected) {
  ...
  if (pkg->name) {
    ...
    if (verbose) {
      ...
      mvwaddch(listpad,index,0, gettext(eflagchars)[pkg->eflag]);
      waddch(listpad, gettext(statuschars)[pkg->status]);
      waddch(listpad,
             /* fixme: keep this feature? */
             /*table[index]->original == table[index]->selected ? ' '
             : */gettext(wantchars)[table[index]->original]);
    
      wattrset(listpad, selected ? selstatesel_attr : selstate_attr);
      waddch(listpad, gettext(wantchars)[table[index]->selected]);
      wattrset(listpad, selected ? listsel_attr : list_attr);
