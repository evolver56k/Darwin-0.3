...
my (@patchextra) = findpatchversion ();
my ($findsep, @findextra) = &findfindversion ();
my ($cpiosep, @cpioextra) = &findcpioversion ();
my ($taroffset) = &findtarversion ();

sub usageversion {
    print STDERR
"Debian GNU/Linux dpkg-source $version.  Copyright (C) 1996, 1997
Ian Jackson and SPI, Inc.  This is free software; see the GNU
...
#system('pwd && ls');
    ...
    if (length ($difffile)) { $expectprefix.= '.orig'; }
    
    if (! length ($tarfile)) { &error ("no tarfile in Files field"); }
    checkstats ($tarfile);
    if (length ($difffile)) { checkstats ($difffile); }

    $tarprefix = checktarsane ("$dscdir/$tarfile", $expectprefix);

    my $tarsubst = quotemeta ($tarprefix);
    my $expectsubst = $expectprefix;
    @filesinarchive = map { s/^$tarsubst/$expectsubst/; $_ } @filesinarchive;
    %dirincluded = map { s/^$tarsubst/$expectsubst/; $_=>1 } (keys %dirincluded);
    %notfileobject = map { s/^$tarsubst/$expectsubst/; $_=>1 } (keys %notfileobject);
    ...
    if (length($difffile)) {
	    ...
	    exec 'patch', '-s', '-t', '-F', '0', '-N', '-p1', '-u', '-V', 'never', @patchextra;
            &syserr("exec patch");
        }
        close(GZIP);
...
sub checkstats {
    ...
    $m =~ s/^([0-9a-f]{32})( +-)?$/$1/ || &failure("md5sum of $f gave bad output \`$m'");
...
sub checktarcpio {
    ...
    $c2 = open (CPIO, "-|");
    if (! defined ($c2)) { &syserr ("fork for cpio"); }
    if ($c2 == 0) {
	...
	# if $tarprefix not set, we are examining the first file in the archive
	if (! $tarprefix) {
	    $tarprefix = ($fn =~ m,([^/]*)[/],)[0];
	    # need to check for multiple dots on some operating systems
	    # empty tarprefix (due to regex failure) will match emptry string
	    if ($tarprefix =~ /^[.]*$/) {
		&error ("tarfile \`$tarfileread' does not extract into a subdirectory"
			. " of the current directory ('$pfn')");
	    }
	}

        if ($fn =~ m/\n/) {
	    &error ("archive file \`$tarfileread' contains object with newline in its name (`$pfn')");
	}
	...
	my $fprefix = substr ($fn, 0, length ($tarprefix));
	    ...
	    &error ("tarfile \`$tarfileread' contains object ($pfn) ".
...
sub tarparse {
  ...
  if (! m;^(\S{10}) (\w{1,8})/(\w{1,8}) +(\d+) (\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}) (.+)$;) {
    &error ("tarfile \`$fn' contains unknown object"
	   . " listed by tar as \`$_'");
  }
  my ($mode, $uid, $gid, $size, $date, $time, $name) = 
    ($1, $2, $3, $4, $5, $6, $7);
  
  $mode =~ s/^([-dpsl])// ||
    &error ("tarfile \`$fn' contains object \`$_' with ".
	    "unknown or forbidden type \`" . substr ($mode, 0, 1) . "'");
  my $type = $&;
  ...
  return ($type, $mode, $uid, $gid, $size, $date, $time, $name);
}
...
sub checktarsane {
	...
	my ($type, $mode, $uid, $gid, $size, $date, $time, $tarfn) 
	  = &tarparse ($tarfileread, $_);
	
	my $ptarfn = $tarfn;
	$ptarfn =~ y/ -~/?/c;

	# fetch name of file as given by cpio
        my $cpiofn = $filesinarchive[$efix++];
	my $pcpiofn = $cpiofn;
	$pcpiofn =~ y/ -~/?/c;

	if ($cpiofn ne $tarfn) {
	    if ((length ($cpiofn) == 99) && (length ($tarfn) >= 99)
		&& (substr ($cpiofn, 0, 99) eq substr ($tarfn, 0, 99))) {
		# this file doesn't match because cpio truncated the name
		# to the first 100 characters.  let it slide for now.
		&warn ("filename \`$pcpiofn' was truncated by cpio;" .
		       " unable to check full pathname");
	    } else {
		&error ("tarfile \`$tarfileread' contains unexpected object".
			" listed by tar as \`$ptarfn'; expected \`$pcpiofn'");
	    }
	}
	...
	if ($tarfn =~ m/[.]{2,}/) {
        ...
        if ($tarfn =~ /\.dpkg-orig$/) {
            &error ("tarfile \`$tarfileread' contains file with name ending in .dpkg-orig \`$ptarfn'");
	}

        if ($mode =~ /[sStT]/ && $type ne "d") {
            &error ("tarfile \`$tarfileread' contains setuid, setgid".
		    " or sticky object \`$ptarfn'");
	}
        ...
        my $dirname = $tarfn;
        if (($dirname =~ s,/[^/]+$,,) && (! defined ($dirincluded{$dirname}))) {
            &error ("tarfile \`$tarfileread' contains object \`$ptarfn' but its containing ".
...
sub forkgzipwrite {
	...
	close (GZIPREAD);
        close (GZIP); 
	open (STDOUT, ">&GZIPFILE") || &syserr ("reopen tar.gz");
        exec ('gzip', '-9');
	&syserr ("exec gzip");
    }
    close (GZIPREAD);
...
sub forkgzipread {
	...
	close (GZIPWRITE);
        close (GZIP); 
	open (STDIN, "<&GZIPFILE") || &syserr ("reopen input file");
        exec ("gunzip"); 
	&syserr ("exec gunzip");
    }
    close (GZIPWRITE);
...
sub addfile {
    ...
    $md5sum =~ s/^([0-9a-f]{32})( +-)?$/$1/ || &failure("md5sum gave bogus output \`$_'");
