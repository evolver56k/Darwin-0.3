...
static void (*script_uncatchsignal[NSCRIPTCATCHSIGNALS]) (int);

static void cu_restorescriptsignals (int argc, void **argv) {
  int i;
  for (i = 0; i < NSCRIPTCATCHSIGNALS; i++) {
    if (signal (script_catchsignallist[i], &script_uncatchsignal[i]) == SIG_ERR) {
...
static void script_catchsignals (void) {
  ...
  onerr_abort++;
  for (i = 0; i < NSCRIPTCATCHSIGNALS; i++) {
    script_uncatchsignal[i] = signal (script_catchsignallist[i], SIG_IGN);
    if (script_uncatchsignal[i] == SIG_ERR) {
...
int maintainer_script_installed(struct pkginfo *pkg, const char *scriptname,
  ...
  arglist= vbuildarglist(scriptname,ap);
  va_end(ap);
  /* supermsg: maintainer_script_installed */
  sprintf(buf,_("%s script"),description);
  ...
  if (stat(scriptpath,&stab)) {
    ...
    /* supermsg: maintainer_script_installed 1 */
...
int maintainer_script_new(const char *scriptname, const char *description,
  ...
  arglist= vbuildarglist(scriptname,ap);
  va_end(ap);
  /* supermsg: maintainer_script_new */
  sprintf(buf,_("%s script"),description);
...
int maintainer_script_alternative(struct pkginfo *pkg,
  ...
  arglist= buildarglist(scriptname,
                        ifok,versiondescribe(&pkg->available.version,
                                             vdew_nonambig),
                        (char*)0);
  /* supermsg: maintainer_script_alternative */
  ...
  if (stat(oldscriptpath,&stab)) {
    ...
    if (WIFEXITED(status)) {
      ...
      fprintf(stderr, _("dpkg: warning - %s killed by signal (%s)%s\n"),
              buf, strsignal(n), WCOREDUMP(status) ? _(", core dumped") : "");
    } else {
