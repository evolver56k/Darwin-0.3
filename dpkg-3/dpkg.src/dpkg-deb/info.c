...
static void info_prepare
(const char *const **argvp, const char **debarp, const char **directoryp, int admininfo) 
{
...
static void info_spew
(const char *debar, const char *directory, const char *const *argv) 
{
  ...
  while ((component= *argv++) != 0) {
    ...
    if (co) {
      ...
      waitsubproc(c1,_("cat component"),0);
...
(const char *debar, const char *directory) 
{
  ...
  cdn= scandir(".", &cdlist, &ilist_select, xalphasort);
...
void do_contents (const char *const *argv)
{
  ...
  if (!(debar = *argv++) || *argv) {
    badusage (_("--contents takes exactly one argument"));
  }
