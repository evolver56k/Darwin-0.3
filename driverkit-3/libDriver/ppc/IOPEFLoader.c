...
#define PCFM_MakeExecutable(addr,len)	flush_cache_v((vm_offset_t)addr, len)
...
PCodeOpen( LogicalAddress container, ByteCount containerSize, PCodeInstance * instance )
{
    ...
    *instance = inst;
    ...
    if( err) LOG( "PEF_OpenContainer = %d\n", err );
...
PCodeClose( PCodeInstance instance )
{
    ...
    if( err) LOG( "PEF_CloseContainer = %d\n", err );
...
PCodeFindExport( PCodeInstance instance, const char * symbolName, LogicalAddress * address, CFragSymbolClass * symbolClass )
{
    ...
    if( err) {
	LOG( "PEF_FindExportedSymbolInfo = %d\n", err );
	return( err);
    }
...
Instantiate( InstanceVars * inst )
{
    ...
    if( err) LOG( "PEF_GetSectionCount = %d\n", err );
    INFO( "Num sects = %d\n", numSects );
    ...
    for( sectionIndex = 0; sectionIndex < numSects; sectionIndex++ )
    {
        ...
        if( err) LOG( "PEF_GetSectionInfo = %d\n", err );
...
//              goto ERROR;
	...
	if ( (section->access & kCFContMemExecuteMask) && (! (section->access & kCFContMemWriteMask)) && isMappable ) {
        ...
        if( err) LOG( "PEF_SetSectionAddress = %d\n", err );
	...
	INFO("unpacking...");
            ...
            if( err) LOG( "PEF_UnpackSection = %d\n", err );
        }
...
OSStatus UnimplementedFunction( UInt32 p1, UInt32 p2, UInt32 p3, UInt32 p4 )
{
...
SatisfyImports( InstanceVars * inst )
{
    ...
    if( err) LOG( "PEF_GetImportCounts = %d\n", err );
    ...
    for( index = 0; index < numLibs; index++ )
    {
        ...
        for( i = 0; i < numLibraries; i++ ) {
            if( strcmp( (char *) curLib->info.libraryName.nameText, Libraries[ i ].name) == 0) {
                curLib->found = &Libraries[ i ];
                break;
            }
        }
    }
    ...
    for( index = 0; index < numSyms; index++ )
    {
        ...
        if( err) LOG( "PEF_GetImportedSymbolInfo = %d\n", err );
	...
	if( symAddr == NULL) {
	    ...
	    symAddr = (TVector *) &stub->pc;
	    stub->pc = UnimplementedFunction;
	    stub->toc = &stub->name[0];
	    strncpy( stub->name, symInfo.symbolName.nameText, 60);
	}

        err = PEF_SetImportedSymbolAddress( cRef, index, symAddr );
        if( err) LOG( "PEF_SetImportedSymbolAddress = %d\n", err );
    }
