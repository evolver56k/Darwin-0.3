...
#ifdef	DDM_DEBUG
	IOInitDDM(1000, "SCSIxpr");
	/* 
	 * Maybe pass this in in argv...
	 */
	IOSetDDMMask(XPR_IODEVICE_INDEX,
		 XPR_SD | XPR_DEVICE | XPR_DISK | XPR_ERR | 
		 XPR_SC | XPR_SCHW | XPR_NDMA);
#endif	DDM_DEBUG
	IOInitGeneralFuncs();
	volCheckInit();
	controllerId = [SCSIController probe:0 deviceMaster:PORT_NULL];
	if(controllerId == nil) {
		printf("Controller Probe failed; exiting.\n");
		exit(1);
	}
	[SCSIDisk probe:controllerId sender:serverId];
	return(0);
}
