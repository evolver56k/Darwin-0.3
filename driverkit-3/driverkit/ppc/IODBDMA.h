...
/* functions for byte reversed loads and stores */

static __inline__ unsigned int ReadSwap32(volatile void * base, volatile unsigned int offset)
{
  unsigned int result;
  __asm__ volatile("lwbrx %0, %1, %2" : "=r" (result) : "r" (offset), "r" (base) : "r0");
  return result;
}

static __inline__ void WriteSwap32(volatile void * base, volatile unsigned int offset, volatile unsigned int data)
{
  __asm__ volatile("stwbrx %0, %1, %2" : : "r" (data), "r" (offset), "r" (base) : "r0" );
}

static __inline__ unsigned int ReadSwap16(volatile void * base, volatile unsigned int offset)
{
  unsigned int result;
  __asm__ volatile("lhbrx %0, %1, %2" : "=r" (result) : "r" (offset), "r" (base) : "r0");
  return result;
}

static __inline__ void WriteSwap16(volatile void * base, volatile unsigned int offset, volatile unsigned int data)
{
  __asm__ volatile("sthbrx %0, %1, %2" : : "r" (data), "r" (offset), "r" (base) : "r0");
}

#define EndianSwap32(value) (   (((value) >> 24) & 0xff) | \
				(((value) >> 8) & 0xff00) | \
				(((value) << 8) & 0xff0000) | \
				(((value) << 24) & 0xff000000) )

#define EndianSwap16(value) (   (((value) >> 8) & 0xff) | \
				(((value) << 8) & 0xff00) )
...
#define IOSetDBDMAChannelRegister(registerSetPtr,field,value)	\
	WriteSwap32(registerSetPtr,offsetof(IODBDMAChannelRegisters,field),value)

#define IOGetDBDMAChannelRegister(registerSetPtr, field)	\
	ReadSwap32(registerSetPtr,offsetof(IODBDMAChannelRegisters, field))
...
#define IOSetDBDMAChannelControl(registerSetPtr,ctlValue)	\
	eieio(); IOSetDBDMAChannelRegister(registerSetPtr,channelControl,ctlValue); eieio()	\
...
#define IOSetDBDMACommandPtr(registerSetPtr,cclPtr)					\
	IOSetDBDMAChannelRegister(registerSetPtr,commandPtrHi,0); eieio();		\
	IOSetDBDMAChannelRegister(registerSetPtr,commandPtrLo,cclPtr); eieio()
...
#define IOSetDBDMAInterruptSelect(registerSetPtr,intSelValue)			\
	IOSetDBDMAChannelRegister(registerSetPtr,interruptSelect,intSelValue); eieio()
...
#define IOSetDBDMABranchSelect(registerSetPtr,braSelValue)	\
	IOSetDBDMAChannelRegister(registerSetPtr,branchSelect,braSelValue); eieio()
...
#define IOSetDBDMAWaitSelect(registerSetPtr,waitSelValue)	\
	IOSetDBDMAChannelRegister(registerSetPtr,waitSelect,waitSelValue); eieio()


/*  These macros are IODBDMADescriptor accessor functions. */

#define IOSetDBDMADescriptor(descPtr,field,value)		\
	WriteSwap32( descPtr, offsetof( IODBDMADescriptor, field), value)

#define IOGetDBDMADescriptor(descPtr,field)	\
	ReadSwap32( descPtr, offsetof( IODBDMADescriptor, field))

#define	IOMakeDBDMAOperation(cmd,key,interrupt,branch,wait,count)					\
			 	( ((cmd) << 28) | ((key) << 24)					\
				| ((interrupt) << 20) | ((branch) << 18) | ((wait) << 16)	\
				| (count))
...
#define IOMakeDBDMADescriptor(descPtr,cmd,key,interrupt,branch,wait,count,addr)	\
	...
	eieio();								\
	IOSetDBDMADescriptor(descPtr,operation, IOMakeDBDMAOperation(cmd,key,interrupt,branch,wait,count));	\
	eieio();
