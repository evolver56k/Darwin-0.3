...
#endif
#ifdef NeXT
#define POP_DROPFILE    ".%s.pop"
#define POP_LOCKFILE	"%s.lock"
	/* POP_TMPSIZE needs to be big enough to hold the string
	 * defined by POP_TMPFILE.
	 */
#define POP_TMPFILE     "tmpXXXXXX"
#else
...
typedef struct  {                               /*  POP parameter block */
    ...
    int                 parm_count;             /*  Number of parameters in 
                                                    parsed list */
#ifdef NeXT
    const char	    **	restricted_users;	/* List of restricted users
						   or NULL if all permitted. */
    char	    **	header_set;		/* Selected headers for HEAD */
#endif /* NeXT */
} POP;
...
extern int  pop_head();
extern int  pop_hset();
extern char *pop_mailfile(char *buf, const char *fmt, ...);
#endif /* NeXT */
