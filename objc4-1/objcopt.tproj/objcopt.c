...
/* Here are some declarations from objcedit.c */
long
...
/*
 * For system call errors the error messages always contain
...
extern void __S(_sel_writeHashTable)(
	int start_addr, 
...
char *argv[];
{
	...
	__S(_sel_writeHashTable)(vm_hashaddr, (char *)selectors, (char *)sel->addr,
				 &table, &size);
	if(swapped)
	...
	for (i = 0; i < nsects; i++) {
		...
		if (s->size > 0) {
		    ...
		    exit(1);
	          ...
	          lseek(fd, s->offset, 0);
	          if((read(fd, (void*)s->reserved1, s->size)) != s->size){
	...
	for (i = 0; i < nsects; i++) {
		...
		if(strncmp(s->sectname, "__class", 16) == 0 ||
		   strncmp(s->sectname, "__meta_class", 16) == 0){
		    ...
		    swap_string_objects((NXConstantStringTemplate *)s->reserved1,
				        s->size);
		}
		    ...
		    swap_objc_ivar_lists((struct objc_ivar_list *)s->reserved1,
                        ...
                        || strncmp(s->sectname, "__cstring_object", 16) == 0
                        || strncmp(s->sectname, "__sel_backref", 16) == 0
                        || strncmp(s->sectname, "__sel_fixup", 16) == 0)
                {
...
enum NXByteOrder target_byte_order)
{
	...
	objc_class->methodLists = (struct objc_method_list **)
		NXSwapLong((long)objc_class->methodLists);
...
{
#if 0
	method_list->methods = (struct objc_method_list *)
		NXSwapLong((long)method_list->methods);
#endif
...
void
swap_string_object(
NXConstantStringTemplate *p,
enum NXByteOrder target_byte_order)
{
    struct _NXConstantStringTemplate *string_object;

	string_object = (struct _NXConstantStringTemplate *)p;
...
)
{
	...
	for (i = 0, j = 0; i < size; i += sizeof(struct _NXConstantStringTemplate), j++)
...
int nsects;
{
	...
	msgsect = getObjcSection(firstobjcsect, nsects, "__message_refs");
  	if (msgsect) {
		...
		for (i = 0; i < cnt; i++) 
			sels[i] = (SEL)__S(_sel_registerName)(
...
print_objc_class:
			...
			if (objc_class->methodLists) {
				print_method_list((struct  objc_method_list*)objc_class->methodLists, 
						firstobjcsect, nsects);
			}

			if(CLS_GETINFO(objc_class, CLS_CLASS)){
                            struct objc_class *objc_meta_class;

                            objc_meta_class = (Class)getObjcData(firstobjcsect, nsects,
                                                                objc_class->isa);
			    objc_class->isa = objc_meta_class;
                            objc_class = objc_meta_class;
		  ...
		  int nsects)
{
	...
	for(i = 0; i < mlist->method_count; i++, method++){
	    ...
	    method->method_name = 
		(SEL)__S(_sel_registerName)((STR)method->method_name);
		  ...
		  int nsects)
{
	...
	for(i = 0; i < mlist->count; i++, method++){
	    ...
	    method->name = (SEL)getObjcData(firstobjcsect, nsects, 
		           			method->name);
	    method->name = 
		(SEL)__S(_sel_registerName)((STR)method->name);
