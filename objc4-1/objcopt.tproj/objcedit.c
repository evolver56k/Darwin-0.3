...
/* Internal routines */
static long	map_input		(char *input);
static long	objc_runtime_setup	(char *input, char *output, char *objc_runtime_setup_contents, unsigned long objc_runtime_setup_size);
static int	qsort_vmaddr		(const struct rep_seg *seg1, const struct rep_seg *seg2);
static int	qsort_fileoff		(const struct rep_seg *seg1, const struct rep_seg *seg2);
static int	qsort_offset		(const struct rep_sect *sect1, const struct rep_sect *sect2);
...
static void	machkern_error		(kern_return_t r, char *format, ...);
...
extern void	swap_segment_command	(struct segment_command *sg, enum NXByteOrder target_byte_order);
extern void	swap_section		(struct section *s, unsigned long nsects, enum NXByteOrder target_byte_order);
extern void	swap_symtab_command	(struct symtab_command *st, enum NXByteOrder target_byte_order);
extern void	swap_symseg_command	(struct symseg_command *ss, enum NXByteOrder target_byte_order);
