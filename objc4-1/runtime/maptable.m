...
#if defined(BUG67896)
static unsigned _hashPrototype (const void *info, const void *data) {
    return hashPrototype (info, data); }

static int _isEqualPrototype (const void *info, const void *data1, const void *data2) {
    return isEqualPrototype (info, data1, data2); }
...
#if defined(BUG67896)
    _hashPrototype, _isEqualPrototype, _NXNoEffectFree, 0
#else
...
unsigned long _mapPtrHash(NXMapTable *table, const void *key) {
    return (((uarith_t) key) >> ARITH_SHIFT) ^ ((uarith_t) key);
}
    
unsigned _mapStrHash(NXMapTable *table, const void *key) {
    unsigned		hash = 0;
...
unsigned _mapObjectHash(NXMapTable *table, const void *key) {
    return [(id)key hash];
}
    
int _mapPtrIsEqual(NXMapTable *table, const void *key1, const void *key2) {
    return key1 == key2;
}

int _mapStrIsEqual(NXMapTable *table, const void *key1, const void *key2) {
...
int _mapObjectIsEqual(NXMapTable *table, const void *key1, const void *key2) {
    return [(id)key1 isEqual:(id)key2];
}
    
void _mapNoFree(NXMapTable *table, void *key, void *value) {}

void _mapObjectFree(NXMapTable *table, void *key, void *value) {
    [(id)key free];
}
