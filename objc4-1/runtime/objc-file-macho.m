 ...
 *	This module is specific to mach-o format files.
 */
...
{
  unsigned size;
  return getsectdatafromheader ((headerType *)head,
				SEG_OBJC, "__runtime_setup", &size);
}

const char *_getObjcStrings(headerType *head, int *nbytes)
{
  void *strings = getsectdatafromheader((headerType *)head,
				    SEG_OBJC,"__meth_var_names",nbytes);
  if (!strings) {
    /* Try old-style strings. */
    strings = getsectdatafromheader((headerType *)head,
				    SEG_OBJC,"__selector_strs", nbytes);
    }
...
{
  void *refs = getsectdatafromheader ((headerType *)head,
				  SEG_OBJC, "__sel_backref", nmess);
  return (SEL *)refs;
}

SEL **_getObjcConflicts(headerType *head, int *nbytes)
{
  char * conflicts = getsectdatafromheader ((headerType *)head,
				  SEG_OBJC, "__sel_conflicts", nbytes);
  if (!conflicts)
    *nbytes = 0;
...
{
  unsigned size;
  void *str = getsectdatafromheader ((headerType *)head,
