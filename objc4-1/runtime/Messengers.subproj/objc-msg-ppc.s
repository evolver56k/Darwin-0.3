...
;		(not found) jumps to cacheMissLabel, registers r8-r10 are
;			garbage, their real values are in the linkage area
;	
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
#endif
	stw		r8,44(r1)			; save r8 through r10 for use as temps
	stw		r9,48(r1)			;
	stw		r10,52(r1)			;
...
; cache hit, r10 == method implementation address
	...
	lwz		r7,40(r1)			; restore r7
#endif
	lwz		r8,44(r1)			; restore r8 through r10
	lwz		r9,48(r1)			;
	lwz		r10,52(r1)			;
.endmacro
...
.macro  MethodTableLookup
	stw		r3, 24(r1)			; save register parameters that
	stw		r4, 28(r1)			; were not already saved by CacheLookup
	stw		r5, 32(r1)			;
	stw		r6, 36(r1)			;
	stw		r7, 40(r1)			;
...
#else
	stfd	f13, -8(r1)			; save the fp register parameters
	stfd	f12, -16(r1)		;
	stfd	f11, -24(r1)		;
	stfd	f10, -32(r1)		;
	stfd	f9, -40(r1)			;
...
.endif
...
#if !defined(KERNEL)
	lfd		f13, -8(r1)			; restore fp register parameters
	lfd		f12, -16(r1)		;
	lfd		f11, -24(r1)		;
	lfd		f10, -32(r1)		;
	lfd		f9, -40(r1)			;
	...
	lwz		r3, 24(r1)			; restore register parameters
	lwz		r4, 28(r1)			;
	lwz		r5, 32(r1)			;
	lwz		r6, 36(r1)			;
	lwz		r7, 40(r1)			;
...
; do profiling when enabled
	...
	cmplwi	r11,kFwdMsgSend		; called via objc_msgSend or objc_msgSend_stret?
...
LMsgForwardSelCmpDone:
	...
	stw		r4, 28(r1)			; (any stack based args already follow this area)
	stw		r5, 32(r1)			;
	stw		r6, 36(r1)			; 
	stw		r7, 40(r1)			;
	stw		r8, 44(r1)
...
LMsgForwardError:
	cmplwi	r11,kFwdMsgSendStret; called via objc_msgSend or objc_msgSend_stret?
