...
#if defined(OBJC_REMOTE_SIGN_BUG)
/*
 * The problem being worked around here is the one where DO 
 ...
 * 
 * The bug needs to get fixed. This workaround is really ugly.
 */
static char *
stripsigns(char *str)
{
...
/* Only write on the strings if we absolutely have to.
 * This change should be forwarded into all the other various
 * flavors of objc.
 */
 	       ++src;
	       ++dst;
...
#warning "Working around OBJC_REMOTE_SIGN_BUG"
  ...
  for (cls = isa; cls; cls = cls->super_class) {
     struct objc_method_list **methodLists = cls->methodLists;
     while ( *methodLists && *methodLists != END_OF_METHODS_LIST ) {
        struct objc_method_list *methods = *methodLists;
        int i;

        for (i = 0; i < methods->method_count; i++)
           if (methods->method_list[i].method_name == aSelector) {
              m = (struct objc_method_description *) &methods->method_list[i];
#if defined(OBJC_REMOTE_SIGN_BUG)
              m->types = stripsigns(m->types);
#endif 
              return (m);
...
id _internal_object_reallocFromZone(Object *anObject, unsigned nBytes, NXZone *zone) 
{
	...
	tmp = anObject->isa; 
	anObject->isa = _objc_getFreedObjectClass ();
	newObject = (Object*)NXZoneRealloc((NXZone*)zone, (void*)anObject, (size_t)nBytes);
	if (newObject) {
		newObject->isa = tmp;
		return newObject;
	}
