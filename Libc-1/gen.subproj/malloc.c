...
#undef set_malloc_singlethreaded
...
#define NXAddRegion _new_NXAddRegion
#define NXCreateChildZone _new_NXCreateChildZone
#define NXCreateZone _new_NXCreateZone
#define NXDefaultMallocZone _new_NXDefaultMallocZone
#define NXMallocCheck _new_NXMallocCheck
#define NXMergeZone _new_NXMergeZone
#define NXRemoveRegion _new_NXRemoveRegion
#define NXZoneCalloc _new_NXZoneCalloc
#define NXZoneFromPtr _new_NXZoneFromPtr
#define NXZonePtrInfo _new_NXZonePtrInfo
#define malloc_singlethreaded _new_malloc_singlethreaded
#define set_malloc_singlethreaded _new_set_malloc_singlethreaded
#define vfree _new_vfree
#endif
...
#if !defined(SHLIB)
    /*
    ** For backwards compatibility, we leave the malloc data structures thread safe
    */
    dolockordebug(1);
#endif
}    
...
#define STARTHUNK (ROUND_SIZE(sizeof(felem_t)*NUMFREES) + sizeof(header_t))
	...
	datap = (header_t *) ((void *)datap + datap->size);
...
header_t *headerp;
	...
	if(regionp->zonep) {
		...
		if((int)ptr >= (int)headerp && 
		    ...
		    printf("start add %p size %ld free %d\n",headerp+1,
		    	headerp->size - sizeof(header_t),headerp->free);
		    printf("zone %p ",regionp->zonep);
	    ...
	    printf("valloced -- start add %p size %d free 0\n",
...
{
zone_t	*zonep = (zone_t *)z;
int	lastspot = 0,nextspot = 0,newsize;
    ...
    */
	...
	felem_t	*freearray;
		...
		!(lastregionp && lastregionp->zonep && 
		    ...
		    if(vm_deallocate(task_self(),((int)headerp & ALIGN_MASK), 
...
zone_t  *zonep;
	...
	if(zonep != 0) {
	    ...
	    type = "VALLOC";
	printf("beginadd %p endadd %p zone %p type %s ",
...
felem_t *lastfreearray;
	...
	if(zonep != 0) {
	    ...
	    type = "VALLOC";
	printf("beginadd %p endadd %p zonep %p type %s ",
		malloc_static.regions[i].beginadd,
		malloc_static.regions[i].endadd, zonep, type);
...
/* For child zones, initial header (dummy) isn't header_t aligned */
	    ...
	    while((int)headerp < (int)malloc_static.regions[i].endadd) {
		printf("add %p size 0x%x free %d lastguyfree %d\n",
		    headerp,headerp->size,headerp->free,
		    headerp->lastguyfree);
		if(headerp->size == 0) {
		    printf("***bad piece at %p size of zero\n",headerp);
		    return;
		}
		if(headerp->size == 4) {
		    printf("-------- extend point --------\n");
...
header_t *headerp,*winner;
    ...
    if(regionp && regionp->zonep) {
	headerp = (header_t *) (ROUND_SIZE((unsigned)regionp->beginadd));
        while(headerp && ((int)headerp < (int)regionp->endadd) && headerp->size) {
	    if(headerp == winner)
		return(1);
	    headerp = (header_t *)((int)headerp + headerp->size);	
	}
...
#if 0
static char *prot_to_str(vm_prot_t protection, char *buf) {
...
static char *inh_to_str(vm_inherit_t inheritance, char *buf) {
    if (inheritance == VM_INHERIT_NONE)
        (void)strcpy(buf, "none");
    if (inheritance & VM_INHERIT_COPY)
        (void)strcpy(buf, "copy");
    if (inheritance & VM_INHERIT_SHARE)
        (void)strcpy(buf, "share");
    return buf;

}

#define FORMATSIZE 100
static char Banner1[] = "   Address     Size Prot Max_prot";
static char Banner2[] = " Inherit Shared Object   Offset";
static char Format1[] = "%#10X %#8X %4s    %4s";
static char Format2[] = "   %5s    %3s    %#3X %#8X";

static void show_vm_regions(void) {
    kern_return_t	kr;
    ...
    region_address = (vm_address_t)0;

    while ((kr = vm_region(task_self(), &region_address, &size, &cur_protection, &max_protection, &inheritance, &shared,  &object_name, &offset)) == KERN_SUCCESS) {
        printf(format, region_address, size, prot_to_str(cur_protection, buf1), prot_to_str(max_protection, buf2),  inh_to_str(inheritance, buf3), shared ? "yes" : "no", object_name, offset);

        if (object_name != PORT_NULL) {
            kr = port_deallocate(task_self(), object_name);
            if (kr != KERN_SUCCESS) {
                mach_error("port_deallocate", kr);
                exit(1);
            }
        }
        region_address += size;
...
void protectMallocData(void) {
    kern_return_t ret;
    show_vm_regions();
    fprintf(stderr, "protecting from %p len %ld\n", malloc_static.regions, malloc_static.max_allocedregion * sizeof(region_t));
    ret = vm_protect(task_self(), (vm_address_t)malloc_static.regions, malloc_static.max_allocedregion * sizeof(region_t),  FALSE, VM_PROT_READ);
    if (ret != KERN_SUCCESS) {
        fprintf(stderr, "[%d] Couldn't protect malloc data regions at %p: %s\n", getpid(), malloc_static.regions,  mach_errormsg(ret));
        return;
    }
    fprintf(stderr, "protecting from %p len %ld\n", malloc_static.zones, malloc_static.max_allocedzone * sizeof(zone_t *));
    ret = vm_protect(task_self(), (vm_address_t)malloc_static.zones, malloc_static.max_allocedzone * sizeof(zone_t *), FALSE,  VM_PROT_READ);
    if (ret != KERN_SUCCESS) {
        fprintf(stderr, "[%d] Couldn't protect malloc data zones at %p: %s\n", getpid(), malloc_static.zones,  mach_errormsg(ret));
        return;
    }
    fprintf(stderr, "[%d] Successfully protected malloc data areas\n", getpid());
}

void unprotectMallocData(void) {
    kern_return_t ret;
    fprintf(stderr, "task_self = %d\n", task_self());
    ret = vm_protect(task_self(), (vm_address_t)malloc_static.regions, malloc_static.max_region * sizeof(region_t),  FALSE, VM_PROT_READ | VM_PROT_WRITE);
    if (ret != KERN_SUCCESS) {
        fprintf(stderr, "[%d] Couldn't unprotect malloc data regions at %p: %s\n", getpid(), malloc_static.regions,  mach_errormsg(ret));
       return;
    }
    ret = vm_protect(task_self(), (vm_address_t)malloc_static.zones, malloc_static.max_zone * sizeof(zone_t *), FALSE, VM_PROT_READ | VM_PROT_WRITE); 
    if (ret != KERN_SUCCESS) {
        fprintf(stderr, "[%d] Couldn't unprotect malloc data zones at %p: %s\n", getpid(), malloc_static.zones,  mach_errormsg(ret));
        return;
    }
    fprintf(stderr, "[%d] Successfully unprotected malloc data areas\n", getpid());
}
#endif
