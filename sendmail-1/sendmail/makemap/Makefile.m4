...
MANROOT=${DESTDIR}ifdef(`confMANROOT', `confMANROOT', `/usr/share/man/man')
...
install-makemap: makemap
	...
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} makemap ${BINDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} makemap.8 ${MAN8}/makemap.${MAN8EXT}')
