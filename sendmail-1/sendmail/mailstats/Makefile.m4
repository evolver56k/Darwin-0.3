...
BINDIR=	ifdef(`confSBINDIR', `confSBINDIR', `/usr/sbin')
...
install-mailstats: mailstats
	...
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} mailstats ${DESTDIR}${BINDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${DESTDIR}${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} mailstats.8 ${DESTDIR}${MAN8}/mailstats.${MAN8EXT}')
