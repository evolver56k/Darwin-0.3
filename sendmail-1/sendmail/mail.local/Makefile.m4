...
BINDIR=	ifdef(`confEBINDIR', `confEBINDIR', `/usr/libexec')
...
install-mail.local: mail.local
	...
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} mail.local ${DESTDIR}${BINDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${DESTDIR}${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} mail.local.8 ${DESTDIR}${MAN8}/mail.local.${MAN8EXT}')
