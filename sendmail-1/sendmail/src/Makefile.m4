...
STDIR=	ifdef(`confSTDIR', `confSTDIR', `/var/log')

# location of sendmail.hf file (usually /usr/share/misc or /usr/lib)
HFDIR=	ifdef(`confHFDIR', `confHFDIR', `/usr/share/misc')
...
install-sendmail: sendmail
	...
	${INSTALL} -d -m 755 ${DESTDIR}${HFDIR}
	${INSTALL} -d -m 755 ${DESTDIR}${STDIR}
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} sendmail ${DESTDIR}${BINDIR}
	for i in ${LINKS}; do rm -f $$i; ln -fs ${BINDIR}/sendmail ${DESTDIR}$$i; done
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m 444 sendmail.hf ${DESTDIR}${HFDIR}
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m 644 sendmail.st ${DESTDIR}${STDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${DESTDIR}${MAN1}
	${INSTALL} -d -m 755 ${DESTDIR}${MAN5}
	${INSTALL} -d -m 755 ${DESTDIR}${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} sendmail.8 ${DESTDIR}${MAN8}/sendmail.${MAN8EXT}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} aliases.5 ${DESTDIR}${MAN5}/aliases.${MAN5EXT}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} mailq.1 ${DESTDIR}${MAN1}/mailq.${MAN1EXT}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} newaliases.1 ${DESTDIR}${MAN1}/newaliases.${MAN1EXT}')
