...
install-praliases: praliases
	...
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} praliases ${DESTDIR}${BINDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${DESTDIR}${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} praliases.8 ${DESTDIR}${MAN8}/praliases.${MAN8EXT}')
