...
BINDIR=	ifdef(`confUBINDIR', `confUBINDIR', `/usr/bin')
...
install-rmail: rmail
	...
	${INSTALL} -c -o ${BINOWN} -g ${BINGRP} -m ${BINMODE} ${STRIPFLAGS} rmail ${DESTDIR}${BINDIR}

install-docs:
ifdef(`confNO_MAN_INSTALL', `dnl',
`	${INSTALL} -d -m 755 ${DESTDIR}${MAN8}
	${INSTALL} -c -o ${MANOWN} -g ${MANGRP} -m ${MANMODE} rmail.8 ${DESTDIR}${MAN8}/rmail.${MAN8EXT}')
