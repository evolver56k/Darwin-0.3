...
void *Page_Create (size_t size)
{
  ...
  kret = vm_allocate (task_self(), &address, size, 1);
...
void Page_AllowAccess (void *address, size_t size)
{
  ...
  kret = vm_protect (task_self(), (vm_address_t) address, size, 0, VM_PROT_READ | VM_PROT_WRITE);
  MACH_CHECK_ERROR (kret);
}
...
void Page_DenyAccess (void *address, size_t size)
{
  ...
  kret = vm_protect (task_self(), (vm_address_t) address, size, 0, VM_PROT_NONE);
  MACH_CHECK_ERROR (kret);
}
...
void Page_Delete (void *address, size_t size)
{
  ...
  kret = vm_deallocate (task_self(), (vm_address_t) address, size);
  MACH_CHECK_ERROR (kret);
}
...
size_t Page_Size (void)
{
  ...
  if (stats == NULL) {
    kret = vm_statistics (task_self(), &stats_data);
    MACH_CHECK_ERROR (kret);
