...
static void ppc_frame_dummy_saved_regs PARAMS 
  ((struct frame_info *frame, struct frame_saved_regs *saved_regs));
...
#define DUMMY_REG_OFFSET \
  (MINIMUM_FRAME_SIZE + ROUND_UP (REGISTER_BYTES, MINIMUM_STACK_ALIGNMENT) - LINK_AREA_SIZE)
...
ppc_push_dummy_frame ()
{
  ...
  assert ((newsp % MINIMUM_STACK_ALIGNMENT) == 0);
  write_sp (newsp);
  ...
  assert (start == (sp - DUMMY_REG_OFFSET));
...
ppc_pop_frame ()
{
  ...
  for (i = 0; i < NUM_REGS; i++) {
    if (frame->ppc_saved_regs->regs[i] != 0) {
      read_memory (frame->ppc_saved_regs->regs[i], &registers [REGISTER_BYTE (i)], REGISTER_RAW_SIZE (i));
    }
  }
     ...
     int type;					/* not used */
{
  ...
  target_addr = fun;
  tocvalue = ppc_find_toc_address (target_addr);
     ...
     CORE_ADDR struct_addr;
{
  ...
  if (struct_return) {
    struct type voidptr_buf;
    struct type *voidptr_type = &voidptr_buf;
    assert (make_pointer_type (builtin_type_void, &voidptr_type) == voidptr_type);
...
{
  char *linkbuf;
  size_t space = ROUND_UP (len, MINIMUM_STACK_ALIGNMENT);
  ...
  *sp = *sp - space;		
  assert ((*sp % MINIMUM_STACK_ALIGNMENT) == 0);
...
void
ppc_frame_cache_saved_regs (frame, retregs)
     struct frame_info *frame;
     struct frame_saved_regs *retregs;
{
  if (! frame->ppc_saved_regs) {
  
    frame->ppc_saved_regs = (struct frame_saved_regs *)
      obstack_alloc (&frame_cache_obstack, sizeof (struct frame_saved_regs));
    assert (frame->ppc_saved_regs != NULL);
    memset (frame->ppc_saved_regs, '\0', sizeof (struct frame_saved_regs));
    
    ppc_frame_saved_regs (frame, frame->ppc_saved_regs);
  }

  if (retregs != NULL) { 
    memcpy (retregs, frame->ppc_saved_regs, sizeof (struct frame_saved_regs));
  }
}
     ...
     struct frame_saved_regs *saved_regs;
{
  ...
  assert (frame != NULL);
  assert (ppc_is_dummy_frame (frame));
  prev = get_prev_frame (frame);
  assert (prev != NULL);

  memset (saved_regs, '\0', sizeof (struct frame_saved_regs));	
  for (i = 0; i < NUM_REGS; i++) {
    saved_regs->regs[i] = prev->frame - DUMMY_REG_OFFSET + REGISTER_BYTE (i);
  }
}
     ...
     struct frame_saved_regs *saved_regs;
{
  ...
  if (! props->frameless) {
    prev_sp = saved_regs->regs [SP_REGNUM] = read_memory_unsigned_integer (frame->frame, 4);
  } else {
    prev_sp = saved_regs->regs [SP_REGNUM] = frame->frame;
  }

  if (props->cr_saved) { saved_regs->regs [CR_REGNUM] = prev_sp + props->cr_offset; }
  if (props->lr_saved) { saved_regs->regs [LR_REGNUM] = prev_sp + props->lr_offset; }
  ...
  if (props->saved_fpr >= 0) {						
    for (i = props->saved_fpr; i < 32; i++) {				
      ...
      saved_regs->regs [FP0_REGNUM + i] = prev_sp + offset;
    }									
  }									
  ...
  if (props->saved_gpr >= 0) {						
    for (i = props->saved_gpr; i < 32; i++) {				
      ...
      saved_regs->regs [GP0_REGNUM + i] = prev_sp + offset;
    }									
  }									
}
