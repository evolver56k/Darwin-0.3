...
/* suspend the mach portion of the inferior task, causing it to have a
   suspend count of 1.  abort() if the inferior no longer exists. */
void next_inferior_suspend_mach PARAMS ((next_inferior_status *s));

/* resume the mach portion of the inferior task, causing it to have a
   suspend count of 0.  abort() if the inferior no longer exists. */
void next_inferior_resume_mach PARAMS ((next_inferior_status *s));
...
void next_thread_list_destroy PARAMS ((thread_process_id *tpid));
