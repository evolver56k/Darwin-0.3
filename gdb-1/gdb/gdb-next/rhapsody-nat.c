...
(struct next_inferior_status *inferior, struct next_exception_data *erequest)
{
  ...
  erequest->header.msg_remote_port = inferior->inferior_old_exception_port;
  erequest->header.msg_local_port = inferior->old_exception_reply_port;
  ...
  ereply.header.msg_local_port = inferior->old_exception_reply_port;
...
(struct next_inferior_status *inferior, struct next_exception_data *erequest)
{
  ...
  assert (erequest->task == inferior->task);
...
(struct next_inferior_status *inferior, task_t task, int pid)
{
  ...
  ret = port_allocate (task_self (), &inferior->inferior_exception_port);
  MACH_CHECK_ERROR (ret);

  ret = port_set_backlog (task_self (), inferior->inferior_exception_port, PORT_BACKLOG_MAX);
  MACH_CHECK_ERROR (ret);

  ret = port_allocate (task_self (), &inferior->old_exception_reply_port);
  MACH_CHECK_ERROR (ret);

  ret = port_set_backlog (task_self (), inferior->old_exception_reply_port, PORT_BACKLOG_MAX);
  MACH_CHECK_ERROR (ret);

  if (inferior_bind_exception_port_flag) {
    ret = task_get_exception_port (inferior->task, &inferior->inferior_old_exception_port);
    MACH_CHECK_ERROR (ret);
    
    ret = task_set_exception_port (inferior->task, inferior->inferior_exception_port);
    MACH_CHECK_ERROR (ret);
  }
