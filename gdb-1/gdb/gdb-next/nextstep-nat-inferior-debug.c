...
void inferior_debug (const char *fmt, ...)
{
  va_list ap;
...
void next_debug_task_port_info (mach_port_t task)
{
  ...
  MACH_CHECK_NOERROR (ret);
  if (ret != KERN_SUCCESS) { return; }

  assert (nnames == ntypes);
...
void next_debug_inferior_status (next_inferior_status *s)
{
  ...
  fprintf (inferior_stderr, "            gdb notify port: 0x%lx\n", (unsigned long) s->notify_port);
  fprintf (inferior_stderr, "              gdb dyld port: 0x%lx\n", (unsigned long) s->dyld_port);
#if !defined (__MACH30__)
  fprintf (inferior_stderr, "    inferior exception port: 0x%lx\n", (unsigned long) s->inferior_exception_port);
  fprintf (inferior_stderr, "inferior old exception port: 0x%lx\n", (unsigned long) s->inferior_old_exception_port);
  fprintf (inferior_stderr, "   old exception reply port: 0x%lx\n", (unsigned long) s->inferior_old_exception_port);
#endif
...
{
  char *s;
  inferior_debug ("exception for thread 0x%lx of task 0x%lx: "
  ...
  inferior_debug ("exception for thread 0x%lx of task 0x%lx: %s\n",
		  (unsigned long) e->thread,
		  (unsigned long) e->task,
		  s);
}
...
void next_debug_notification_message (struct next_inferior_status *inferior, msg_header_t *msg)
{
	...
	inferior_debug ("next_mach_process_message: deletion message for task port 0x%lx\n", 
			(unsigned long) dmsg->not_port);
      } else {
	inferior_debug ("next_mach_process_message: deletion message for unknown port 0x%lx; ignoring\n", 
			(unsigned long) dmsg->not_port);
      }
    } else {
...
_initialize_next_inferior_debug ()
{
  ...
  cmd = add_set_cmd ("debug-inferior", class_obscure, var_boolean, 
