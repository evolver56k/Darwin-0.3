...
char *dyld_entry_string (struct dyld_objfile_entry *e)
{
  char *ret = NULL;
  char *ret2 = NULL;
  ...
  if (e->loaded_flag) {
    if (e->loaded_from_memory) {
      assert (! e->loaded_addrisoffset);
      assert (e->loaded_addr == e->loaded_memaddr);
	...
	if (e->image_addr_valid) {
	  ...
	  asprintf (&ret, "\"%s\" at 0x%lx (offset %s)", e->loaded_name,
		    (unsigned long) e->image_addr, offstr);
	  free (offstr);
	} else {
	  char *offstr = dyld_offset_string ((unsigned long) e->loaded_offset);
	  asprintf (&ret, "\"%s\" (offset %s)", e->loaded_name, offstr);
	  free (offstr);
	}
      } else {
	if (e->dyld_valid) {
	  ...
	  if (e->image_addr_valid) {
	    ...
	    asprintf (&ret, "\"%s\" at 0x%lx (offset %s)", 
		      e->loaded_name, (unsigned long) e->loaded_addr, offstr);
	    free (offstr);
	  } else {
	    asprintf (&ret, "\"%s\" at 0x%lx", e->loaded_name,
...
void dyld_print_shlib_info (struct dyld_objfile_info *s) 
{
  ...
  for (i = 0; i < s->nents; i++) {
	...
	baselen = 1;
      ...
      dyld_library_basename (name, &tfname, &tfnamelen, NULL);
