     ...
     ppc_function_properties *props;
{
  ...
  assert ((start % 4) == 0);	/* instructions must be word-aligned */
  assert ((end == INVALID_ADDRESS) || (end % 4) == 0);	/* instructions must be word-aligned */
  assert ((end >= start) || (end == INVALID_ADDRESS));

  for (pc = start; (end == INVALID_ADDRESS) || (pc < end); pc += 4) {
     ...
     ppc_function_boundaries *reply;
{
  ...
  assert (reply->prologue_start != INVALID_ADDRESS);
     ...
     struct ppc_function_boundaries *retbounds;
{
  if (! frame->bounds) {
    ...
    if (ppc_is_dummy_frame (frame)) {
      ...
      frame->bounds = (struct ppc_function_boundaries *)
	obstack_alloc (&frame_cache_obstack, sizeof (ppc_function_boundaries));
      assert (frame->bounds != NULL);
      
      frame->bounds->prologue_start = INVALID_ADDRESS;
      frame->bounds->body_start = INVALID_ADDRESS;
      frame->bounds->epilogue_start = INVALID_ADDRESS;
      frame->bounds->function_end = INVALID_ADDRESS;
	...
	obstack_alloc (&frame_cache_obstack, sizeof (ppc_function_boundaries));
      assert (frame->bounds != NULL);
      memcpy (frame->bounds, &lbounds, sizeof (ppc_function_boundaries));
    }
  }

  if (retbounds != NULL) { 
    memcpy (retbounds, frame->bounds, sizeof (ppc_function_boundaries));
  }
     ...
     struct ppc_function_properties *retprops;
{
  if (! frame->props) {
	...
	obstack_alloc (&frame_cache_obstack, sizeof (ppc_function_properties));
      ...
      if ((frame->bounds->prologue_start % 4) != 0) { return -1; }
      if ((frame->pc % 4) != 0) { return -1; }
      ppc_parse_instructions (frame->bounds->prologue_start, frame->pc, &lprops);
  
      frame->props = (struct ppc_function_properties *)
	obstack_alloc (&frame_cache_obstack, sizeof (ppc_function_properties));
      assert (frame->props != NULL);
      memcpy (frame->props, &lprops, sizeof (ppc_function_properties));
    }
  }

  if (retprops != NULL) {
    memcpy (retprops, frame->props, sizeof (ppc_function_properties));
  }
