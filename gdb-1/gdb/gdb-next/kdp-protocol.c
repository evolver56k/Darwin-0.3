  ...
  *plen = 0;
  ...
  if (p->hdr.is_reply) {
    ...
    case KDP_READREGS: {
      ...
      len = reglen + 12;
      CHECK_LEN_MAX (len, maxlen);
      if ((reglen % REGISTER_SIZE) != 0) { return RR_IP_ERROR; }
      write32u (s + 8, p->readregs_reply.error, c->bigendian);
      for (i = 0; i < (reglen / REGISTER_SIZE); i++) {
	write32u (s + 12 + (i * REGISTER_SIZE), 
...
#define CHECK_PLEN_MINLEN(plen, len) \
  ...
  if (p->hdr.is_reply) {
    ...
    case KDP_READREGS: { 
      ...
      if ((reglen % REGISTER_SIZE) != 0) { 
	c->logger (KDP_LOG_ERROR, 
		   "length of register data (%u) is not an integer multiple of REGISTER_SIZE (%u)\n",
		   reglen, REGISTER_SIZE);
	return RR_IP_ERROR; 
      }
      p->readregs_reply.nbytes = reglen;
      for (i = 0; i < (reglen / REGISTER_SIZE); i++) {
	((unsigned long *) p->readregs_reply.data)[i] =
	  read32u (s + 12 + (i * REGISTER_SIZE), c->bigendian);
      }
      break;
    }
    case KDP_WRITEREGS:
	...
	return RR_IP_ERROR; 
      ...
      for (i = 0; i < (reglen / REGISTER_SIZE); i++) {
	((unsigned long *) p->writeregs_req.data)[i] =
	  read32u (s + 16 + (i * REGISTER_SIZE), c->bigendian);
      }
      break;
    }
    case KDP_LOAD:
