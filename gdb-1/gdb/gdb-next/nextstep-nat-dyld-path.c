...
static char *search_for_name_in_path (const char *name, const char *path)
{
  ...
  namelen = strlen (name);
  dylib_name = xmalloc (strlen (name) + strlen (path) + 2);
...
static const char *get_framework_pathname (const char *name, int with_suffix)
{
...
void dyld_library_basename (const char *path, const char **s, unsigned int *len, int *is_framework)
{
  const char *p = NULL;
  const char *q = NULL;
  ...
  p = get_framework_pathname (path, 1);
  if (p != NULL) {
...
char *dyld_resolve_image (const struct dyld_path_info *d, const char *dylib_name)
{
  ...
  framework_name = get_framework_pathname (dylib_name, 0);
  framework_name_suffix = get_framework_pathname (dylib_name, 1);
  ...
  if (d->framework_path != NULL) {
	...
	(framework_name, d->framework_path);
    ...
    if (framework_name_suffix != NULL) {
      framework_path = search_for_name_in_path
	(framework_name_suffix, d->framework_path);
      if (framework_path != NULL) {
	return framework_path;
      }
    }
  ...
  if (d->fallback_framework_path != NULL) {
    ...
    if (framework_name != NULL){
      framework_path = search_for_name_in_path
	(framework_name, d->fallback_framework_path);
      if (framework_path != NULL) {
	return framework_path;
      }
    }
    ...
    if (framework_name_suffix != NULL) {
      framework_path = search_for_name_in_path
	(framework_name_suffix, d->fallback_framework_path);
      if (framework_path != NULL) {
	return framework_path;
      }
    }
  ...
  if (d->fallback_library_path != NULL) {
    framework_path = search_for_name_in_path (library_name, d->fallback_library_path);
    if (framework_path != NULL) {
      return framework_path;
    }
  }
...
void dyld_init_paths (dyld_path_info *d)
{
  ...
  if ((getuid() == geteuid()) && (getgid() == getegid())) {
    ...
    d->framework_path = get_in_environ (inferior_environ, "DYLD_FRAMEWORK_PATH");
    if (d->framework_path != NULL) { d->framework_path = strsave (d->framework_path); }

    d->library_path = get_in_environ (inferior_environ, "DYLD_LIBRARY_PATH");
    if (d->library_path != NULL) { d->library_path = strsave (d->library_path); }

    d->fallback_framework_path = get_in_environ (inferior_environ, "DYLD_FALLBACK_FRAMEWORK_PATH");
    if (d->fallback_framework_path != NULL) { d->fallback_framework_path = strsave (d->fallback_framework_path); }

    d->fallback_library_path = get_in_environ (inferior_environ, "DYLD_FALLBACK_LIBRARY_PATH");
    if (d->fallback_library_path != NULL) { d->fallback_library_path = strsave (d->fallback_library_path); }

    d->insert_libraries = get_in_environ (inferior_environ, "DYLD_INSERT_LIBRARIES");
    if (d->insert_libraries != NULL) { d->insert_libraries = strsave (d->insert_libraries); }
  }
  ...
  if (d->fallback_framework_path == NULL) {
    d->fallback_framework_path = xmalloc (strlen (default_fallback_framework_path) + strlen (home) + 1);
    sprintf (d->fallback_framework_path, default_fallback_framework_path, home);
  }

  if (d->fallback_library_path == NULL) {
    d->fallback_library_path = xmalloc (strlen (default_fallback_library_path) + strlen (home) + 1);
