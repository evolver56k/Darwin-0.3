 ...
 exception_type_t exception_type, exception_data_t exception_data, mach_msg_type_number_t data_count)
{
  ...
  assert ((inferior->exception.masks[original_port_index] & (1 << exception_type)) != 0);

  port = inferior->exception.ports[original_port_index];
  behavior = inferior->exception.behaviors[original_port_index];
  flavor = inferior->exception.flavors[original_port_index];
  ...
  case EXCEPTION_DEFAULT:
    inferior_debug ("forwarding to exception_raise\n");
  ...
  case EXCEPTION_STATE:
    inferior_debug ("forwarding to exception_raise_state\n");
  ...
  case EXCEPTION_STATE_IDENTITY:
    inferior_debug ("forwarding to exception_raise_state_identity\n");
  ...
  default:
    inferior_debug ("forward_exception got unknown behavior\n");
    break;
  }
...
{
  inferior_debug ("catch_exception_raise_state called\n");
  return KERN_FAILURE;
}
 ...
 thread_state_t out_state, mach_msg_type_number_t out_state_count)
{
  ...
  inferior_debug ("catch_exception_raise_state_identity called\n");
 ...
 mach_msg_type_number_t data_count)
{
  ...
  inferior_debug ("catch_exception_raise called\n");
  inferior_debug ("\tport: %#x\n\tthread_port: %#x\n\ttask_port: %#x\n", port, thread_port, task_port);
  inferior_debug ("\texception_type: %#x\n\tdata_count: %#x\n", exception_type, data_count);
  for (i = 0; i < data_count; ++i) {
    inferior_debug ("\texception_data[%d]: %#x\n", i, exception_data[i]);
  }
...
(struct next_inferior_status *inferior, task_t task, int pid)
{
  ...
  ret = mach_port_allocate (mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &inferior->inferior_exception_port);
  MACH_CHECK_ERROR (ret);
  ret = mach_port_insert_right (mach_task_self(), inferior->inferior_exception_port,
				inferior->inferior_exception_port, MACH_MSG_TYPE_MAKE_SEND);
  MACH_CHECK_ERROR (ret);

  ret = mach_port_allocate (mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &inferior->old_exception_reply_port);
  MACH_CHECK_ERROR (ret);
  ret = mach_port_insert_right (mach_task_self(), inferior->old_exception_reply_port,
				inferior->old_exception_reply_port, MACH_MSG_TYPE_MAKE_SEND);
  MACH_CHECK_ERROR (ret);

  if (inferior_bind_exception_port_flag) {

    inferior->exception.count = (sizeof (inferior->exception.ports) / sizeof (inferior->exception.ports[0]));
    ret = task_get_exception_ports 
      (inferior->task,
       EXC_MASK_ALL & ~(EXC_MASK_MACH_SYSCALL | EXC_MASK_SYSCALL | EXC_MASK_RPC_ALERT),
       inferior->exception.masks, &inferior->exception.count, inferior->exception.ports,
       inferior->exception.behaviors, inferior->exception.flavors);
    MACH_CHECK_ERROR (ret);

    ret = task_set_exception_ports
      (inferior->task,
       EXC_MASK_ALL & ~(EXC_MASK_MACH_SYSCALL | EXC_MASK_SYSCALL | EXC_MASK_RPC_ALERT),
       inferior->inferior_exception_port, EXCEPTION_DEFAULT, THREAD_STATE_NONE);
    MACH_CHECK_ERROR (ret);
  }

  inferior->last_thread = next_primary_thread_of_task (inferior->task);

  next_signal_thread_create (&inferior->signal_status, inferior->signal_port, inferior->pid);
}
