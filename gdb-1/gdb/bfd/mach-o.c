     ...
     bfd *abfd;
{
...

  {
    ...
    for (;;) {
      ...
      bfd_vma nread = bfd_read ((PTR) buf, 1, 1024, ibfd);
      if (nread == 0) { break; }
...
static const char *bfd_mach_o_flavour_string (flavour)
     ...
     bfd_mach_o_load_command *command;
{
  ...
  while (offset != command->len) {
    ...
    if (bfd_h_get_32 (abfd, buf + 4) < 0) { return -1; }
  ...
  while (offset != command->len) {
    ...
    if (cmd->flavours[nflavours].size < 0) { return -1; }
  ...
  for (i = 0; i < nflavours; i++) {
    ...
    flavourstr = bfd_mach_o_flavour_string (cmd->flavours[i].flavour);
...
int
bfd_mach_o_scan_start_address (abfd, header)
     bfd *abfd;
{
  bfd_mach_o_data_struct *mdata = abfd->tdata.mach_o_data;
