...
/* If a machine allows frameless functions, it should define a macro
   FRAMELESS_FUNCTION_INVOCATION(FI) in param.h.  FI is the struct
   frame_info for the frame, and the macro should evaluate to non-zero
   if FI represents a frameless function invocation.  */
     ...
     struct frame_info *frame;
{
  ...
  if (frame->next)
    sigcontext_addr = read_memory_unsigned_integer
      (FRAME_ARGS_ADDRESS (frame->next)
       + FRAME_ARGS_SKIP
       + sigcontext_offs,
       ptrbytes);
  else
