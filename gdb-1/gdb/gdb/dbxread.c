...
#ifdef NM_NEXTSTEP
extern int scatterload_hack;
#else
static int scatterload_hack = 0;
#endif
  ...
  if (STRING_TABLE_OFFSET == 0)
      ...
      else
	{
	  ...
	  if (DBX_STRINGTAB_SIZE (objfile) < sizeof (size_temp))
	    error ("invalid string table size (%d bytes).",
...
#define SET_NAMESTRING()\
  if (((unsigned)CUR_SYMBOL_STRX + file_string_table_offset) >=		\
    ...
    if ((prefix != NULL) && (prefix[0] != '\0')) {			\
      assert ((strlen (s) + strlen (prefix) + 1) < 4096);		\
      sprintf (namebuf, "%s%s", prefix, s);                             \
      namestring = namebuf;                                             \
    } else {								\
      namestring = s;							\
    }									\
...
#define START_PSYMTAB(ofile,secoff,fname,low,symoff,global_syms,static_syms)\
  start_psymtab(ofile, secoff, fname, low, symoff, global_syms, static_syms, prefix)
     ...
     int text_size;
{
  ...
  const char *prefix = objfile->prefix;
...
start_psymtab (objfile, section_offsets,
	       filename, textlow, ldsymoff, global_syms, static_syms, prefix)
     struct objfile *objfile;
     struct section_offsets *section_offsets;
     char *filename;
     CORE_ADDR textlow;
...
void
process_one_symbol (type, desc, valu, name, prefix, section_offsets, objfile)
     int type, desc;
     CORE_ADDR valu;
     char *name;
     const char *prefix;
      ...
      if (name)
	{
	  ...
	  switch (deftype)
	      ...
	      if (context_stack_depth > 0)
		{
		  new = pop_context ();
		  /* Make a block for the local symbols within.  */
		  if (scatterload_hack && (objfile->flags & OBJF_REORDERED))
		    finish_block (new->name, &local_symbols, new->old_blocks,
				  new->start_addr, (CORE_ADDR) -1, objfile);
		  else
		    finish_block (new->name, &local_symbols, new->old_blocks,
				  new->start_addr, valu, objfile);
		}
...
stabsect_build_psymtabs (objfile, section_offsets, mainline, stab_name,
			 stabstr_name, text_name, data_name, bss_name)
     struct objfile *objfile;
     struct section_offsets *section_offsets;
     int mainline;
     char *stab_name;
     ...
     char *bss_name;
{
  ...
  if (mmap_strtabflag) {
    ...
    val = bfd_get_section_contents_in_window (sym_bfd, /* bfd */
