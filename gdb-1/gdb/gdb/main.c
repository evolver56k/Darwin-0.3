...
/* We have probably been invoked from emacs.  Disable window interface.  */
  ...
  if (homedir)
    {
      ...
      if ((!inhibit_gdbinit)
	  && (memcmp ((char *) &globalbuf, (char *) &homebuf, sizeof (struct stat)) != 0))
	{
	  if (!SET_TOP_LEVEL ())
  ...
  stat (gdbinit, &cwdbuf);
  if ((!inhibit_gdbinit)
      && (memcmp ((char *) &globalbuf, (char *) &cwdbuf, sizeof (struct stat)) != 0)
      && (memcmp ((char *) &homebuf, (char *) &cwdbuf, sizeof (struct stat)) != 0))
    if (!inhibit_gdbinit)
      {
	if (!SET_TOP_LEVEL ())
