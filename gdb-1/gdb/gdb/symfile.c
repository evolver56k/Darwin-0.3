...
#if HAVE_MMAP
static boolean mmap_symfilesflag = 0;
#endif /* HAVE_MMAP */
...
void
syms_from_objfile (objfile, addr, addrisoffset, mainline, verbo)
     struct objfile *objfile;
     CORE_ADDR addr;
     int addrisoffset;
     int mainline;
     int verbo;
{
  ...
  if (!mainline && !addrisoffset)
    {
#if TM_NEXTSTEP
      lowest_sect = objfile->obfd->sections;
#else
...
struct objfile *
symbol_file_add (name, from_tty, addr, addrisoffset, mainline, mapped, mapaddr, readnow, user_loaded, is_solib, prefix)
     const char *name;
     int from_tty;
     CORE_ADDR addr;
     int addrisoffset;
     ...
     const char *prefix;
{
  ...
  return symbol_file_add_bfd 
    (abfd, from_tty, addr, addrisoffset,
     mainline, mapped, mapaddr,
     readnow, user_loaded, is_solib, prefix);
}


struct objfile *
symbol_file_add_bfd (abfd, from_tty, addr, addrisoffset, mainline, mapped, mapaddr, readnow, user_loaded, is_solib, prefix)
     bfd *abfd;
     int from_tty;
     CORE_ADDR addr;
     int addrisoffset;
     ...
     const char *prefix;
{
  ...
  objfile = allocate_objfile (abfd, mapped, mapaddr, user_loaded, is_solib);
  ...
  if ((objfile -> flags & OBJF_MAPPED) && (objfile -> flags & OBJF_SYMS))
      ...
      if (from_tty || info_verbose)
	{
      ...
      syms_from_objfile (objfile, addr, addrisoffset, mainline, from_tty);
    }
	  ...
	  if (STREQ (*argv, "-mapped"))
		...
		if (! from_tty) {
		  ...
		  gdb_flush (gdb_stdout);
                ...
                symbol_file_add (name, from_tty, (CORE_ADDR)0, 0, 1, mapped,
				 0, readnow, 1, 0, NULL);
		if (! from_tty) {
		  printf_filtered ("done.\n");
		  gdb_flush (gdb_stdout);
     ...
     int from_tty;
{
  ...
  while ((*args != '\000') && (name == NULL))
      ...
      else if (STREQ (arg, "-mapped"))
	{
      ...
      else if (STREQ (arg, "-mapaddr"))
	{
	  if (args[1] == NULL)
	    error ("usage: add-symbol-file -mapaddr <address> ...");
	  mapaddr = parse_and_eval_address (args[1]);
	  args++;
	}
  ...
  if (*args != '\000')
    {
      ...
      symbol_file_add (name, 0, text_addr, 0, 0, mapped, mapaddr, readnow, 1, 0, NULL);
    }
  else
    {
#if 0
...
#else
      symbol_file_add (name, 0, 0, 1, 0, mapped, mapaddr, readnow, 1, 0, NULL);
#endif
    }
...
reread_symbols ()
{
  ...
  for (objfile = object_files; objfile; objfile = objfile->next) {
      ...
      if (new_modtime != objfile->mtime)
	{
	  ...
	  warning ("unable to re-read symbols");
	  continue;
