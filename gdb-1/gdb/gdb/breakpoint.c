	...
	if (!found_a_breakpoint++)
	  {
	    if (annotation_level >= 2) {
	      /* send over all the header and use full words for them */
	      annotate_breakpoints_headers ();
	      annotate_field (0);
	      printf_filtered ("Number");
	      annotate_field (1);
	      printf_filtered ("Type");
	      annotate_field (2);
	      printf_filtered ("Disposition");
	      annotate_field (3);
	      printf_filtered ("Enabled");
	      if (addressprint) {
                annotate_field (4);
     ...
     PTR bint;
{
  ...
  switch (b->type)
      ...
      if (b->addr_string == NULL)
	{
	  ...
	  warning ("Unable to reset breakpoint %d; deleting (no address string available)", b->number);
	  delete_breakpoint (b);
	  return 0;
	}
