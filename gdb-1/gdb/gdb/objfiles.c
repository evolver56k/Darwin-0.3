...
open_mapped_objfile (abfd, mapped, mapaddr)
     bfd *abfd;
     int mapped;
     CORE_ADDR mapaddr;
{
     ...
     CORE_ADDR mapaddr;
{
  ...
  if (mapaddr == NULL)
...
#if TM_NEXTSTEP
      lowest_sect = abfd->sections;
#else
      lowest_sect = bfd_get_section_by_name (abfd, ".text");
      if (lowest_sect == NULL)
	bfd_map_over_sections (abfd, find_lowest_section,
			       (PTR) &lowest_sect);
#endif
	...
	warning ("no loadable sections found in added symbol-file %s",
      ...
      else if ((bfd_get_section_flags (abfd, lowest_sect) & SEC_CODE)
	       == 0)
	/* FIXME-32x64--assumes bfd_vma fits in long.  */
	warning ("lowest section in %s is %s at 0x%lx",
		 objfile->name,
		 bfd_section_name (abfd, lowest_sect),
		 (unsigned long) bfd_section_vma (abfd, lowest_sect));

      if (lowest_sect)
	mapaddr = bfd_section_vma (abfd, lowest_sect) + 0x80000000;
      else
	{
...
struct objfile *
allocate_objfile (abfd, mapped, mapaddr, user_loaded, is_solib)
     bfd *abfd;
     int mapped;
     CORE_ADDR mapaddr;
     int  user_loaded;
     int  is_solib;
{
  ...
  if (abfd != NULL)
  {
    objfile = open_mapped_objfile (abfd, mapped, mapaddr);
    if ((objfile == NULL) && mapped) {
      objfile = create_mapped_objfile (abfd, mapped, mapaddr);
    }
  }
