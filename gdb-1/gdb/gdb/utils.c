...
#elif defined(_MSC_VER) && !defined (NeXT_PDO) /* should test for wingdb instead? */
  ...
  if (md == NULL) 
    {
      int fd;
      char buf[64];
      sprintf (buf, "/tmp/gdb.mmalloc.XXXXXX");
      
      fd = mkstemp (buf);
      if (fd < 0) 
	fatal_dump_core ("unable to create default mmalloc allocator: %s",
			 strerror (errno));
#if defined (__APPLE__) && defined (__MACH__)
      md = mmalloc_attach (fd, (void *) 0x20000000);
#else
      md = mmalloc_attach (fd, (void *) 0x0);
#endif
      if (md == NULL)
	fatal_dump_core ("unable to create default mmalloc allocator");

      if (unlink (buf) != 0) {
	fatal_dump_core ("unable to unlink map file for default mmalloc allocator: %s",
			 strerror (errno));
      }
    }
#endif
