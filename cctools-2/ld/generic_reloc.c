...
long pcrel_at_end_of_disp)
{
	...
	for(i = 0; i < section_map->s->nreloc; i++){
		...
		if(undefined_map == NULL){
		    ...
		    if((nlists[symbolnum].n_type & N_TYPE) != N_UNDF){
			error_with_cur_obj("r_symbolnum (%lu) field of external"
			    " relocation entry %lu in section (%.16s,%.16s) "
		    ...
		    fatal("internal error, in generic_reloc() symbol index %lu "
...
update_reloc:
		...
		if(r_extern){
		     ...
		     * defined (not undefined or common) and if the output file
		     * is not MH_DYLIB or the symbol is a private extern it is
		     * changed to a local relocation entry using the section
		     * that symbol is defined in.  If still undefined then the
