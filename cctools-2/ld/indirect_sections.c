...
account_for_size:
	...
	for(i = 0; i < s->nreloc; i++){
	     ...
	     * for one of the pointers and therefore the offset must be a
	     * multiple of 4 (the size of a pointer), have an r_length field of
	     * 2 (long) and a r_pcrel field of 0 (FALSE).
	     */
	    if(section_type == S_LAZY_SYMBOL_POINTERS){
		if(r_address % 4 != 0){
		...
		if(r_extern == 0){
			...
			error_with_cur_obj("r_symbolnum (%lu) field of local "
			    ...
			    "out of range", r_symbolnum, i,s->segname,
			    s->sectname);
			continue;
		    }
		}
			...
			if((nlists[r_symbolnum].n_type & N_TYPE) != N_UNDF){
			    error_with_cur_obj("r_symbolnum (%lu) field of "
				"external relocation entry %lu in section "
				"(%.16s,%.16s) refers to a non-undefined "
				"symbol",r_symbolnum,i,s->segname,s->sectname);
			    continue;
			}
		...
		if(undefined_map == NULL){
		    ...
		    print_obj_name(cur_obj);
		    fatal("internal error, in indirect_section_merge() symbol "
		...
		if(filetype == MH_DYLIB){
			...
			if(merged_symbol->error_flagged_for_dylib == 0){
				...
				"for MH_DYLIB output file to symbol: %s from a "
				"symbol stub section (section (%.16s,%.16s) "
				"relocation entry: %lu)",
			...
			if(merged_symbol->error_flagged_for_dylib == 0){
				...
				"private extern symbol) from a symbol stub "
				"section (section (%.16s,%.16s) relocation "
	...
	/*
	 * Third deal with the symbol table entries for local symbols in this
	 * section in this object file (globals are not allowed and
	 * merge_symbols() in pass1 has checked this).  Now that it has been
	...
	for(nsect = 0; nsect < cur_obj->nsection_maps; nsect++)
		...
		break;
	nsect++; /* section number start at 1 (not zero) */
	/* set up a pointer to the string table */
	strings = (char *)(cur_obj->obj_addr + cur_obj->symtab->stroff);
	for(i = 0; i < cur_obj->symtab->nsyms; i++){
	    if((nlists[i].n_type & N_EXT) == 0 && 
	       (nlists[i].n_type & N_TYPE) == N_SECT && 
	       nlists[i].n_sect == nsect && 
	       (strip_level == STRIP_NONE ||
	        is_output_local_symbol(nlists[i].n_type,
				       nlists[i].n_un.n_strx == 0 ? "" :
				       strings + nlists[i].n_un.n_strx))){

		if(fine_reloc_offset_in_output(section_map,
			nlists[i].n_value - s->addr) == FALSE){
		    nlocal_symbols--;
		    cur_obj->nlocalsym--;
		    cur_obj->symbols_removed = TRUE;
		}
		if(strip_level != STRIP_NONE){
		    local_string_size -= nlists[i].n_un.n_strx == 0 ? 0:
					 strlen(strings + nlists[i].n_un.n_strx)
					 + 1;
		}
	    }
	}
			...
			*q ;
			...
			for(i = 0; i < indirect_item_block->used; i++){
				...
				if((ms->s.flags & SECTION_TYPE) ==
				   ...
				   indirect_item->merged_symbol->nlist.n_type &
				   N_PEXT){
