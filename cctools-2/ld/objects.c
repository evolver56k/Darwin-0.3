...
static struct fine_reloc *fine_reloc_for_input_offset(
...
unsigned long output_base_address)
{
	...
	if(fine_reloc->local_symbol == TRUE){
		...
		fatal("internal error, fine_reloc_output_address() called with "
		      ...
		      "local_symbol is TRUE but it's not in a S_SYMBOL_STUBS or"
		      " a S_LAZY_SYMBOL_POINTERS section");
		return(0);
	    }
	}
...
{
    int i, j;
    unsigned long stride, section_type;

	if(map->nfine_relocs == 0)
	    fatal("internal error, fine_reloc_offset_in_output_for_output_"
		  "offset() called with a map->nfine_relocs == 0");
	/*
	 * Find the fine_reloc entry for this output offset.  This is a linear
	 * search as the fine_relocs are sorted by input_offset.
	 */
	stride = 0;
	section_type = map->s->flags & SECTION_TYPE;
	if(section_type == S_LAZY_SYMBOL_POINTERS ||
	   section_type == S_NON_LAZY_SYMBOL_POINTERS)
	    ...
	    stride = map->s->reserved2;
	else
	    fatal("internal error, fine_reloc_offset_in_output_for_output_"
	      "offset() called with a section map for a non-indirect section");
	 ...
	 * output_offset is not in the output.
	 */
	...
	for(i = 0; i < map->nfine_relocs; i++){
	    if(map->fine_relocs[i].use_contents == FALSE)
		continue;
	    if(output_offset >= map->fine_relocs[i].output_offset &&
	       output_offset <  map->fine_relocs[i].output_offset + stride){
		j = i;
		break; 
	    }
	}
	if(j == -1)
	    fatal("internal error, fine_reloc_offset_in_output_for_output_"
		  "offset() called with bad output_offset");
	return((enum bool)(map->fine_relocs[j].use_contents));
}
...
unsigned long input_offset)
{
	...
	if(fine_reloc->local_symbol == TRUE){
		...
		fatal("internal error, fine_reloc_output_sectnum() called with "
		      "an input_offset which maps to a fine_reloc where "
		      "local_symbol is TRUE but it's not in a S_SYMBOL_STUBS or"
		      " a S_NON_LAZY_SYMBOL_POINTERS section");
		index = 0;
	    }
	    nlists = (struct nlist *)(cur_obj->obj_addr +
				      cur_obj->symtab->symoff);
