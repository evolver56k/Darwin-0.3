 ...
 * "-arch <arch_flag>" (if given) in /usr/libexec/<arch_flag>/as or in
 * /usr/local/libexec/<arch_flag>/as.  Or runs the assembler for the host
 * architecture as returned by get_arch_from_host().  The driver only checks
 * to make sure their are not multiple arch_flags and then passes all flags to
 * the assembler it will run.
 */
#include "stdio.h"
#include "stdlib.h"
#include "string.h"
...
char **envp)
{
    const char *LIB = "/usr/libexec/";
    const char *LOCALLIB = "/usr/local/libexec/";
    const char *AS = "/as";
