...
void (*dyld_event_routine)(struct dyld_event event))
{
	...
	k = user_dyld_debug_add_event_subscriber(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS)
            d = DYLD_FAILURE;
	/* BUG: the value of d is never used */
#else
...
struct _dyld_debug_task_state *state)
{
    ...
    struct dyld_data_section data;
    thread_port_array_t threads;
    unsigned int count, count1, trys;
	...
	if(data.debug_thread == MACH_PORT_NULL ||
	   data.debug_port == MACH_PORT_NULL){
...
#ifdef CORE
			     TRUE, /* function parameter, start_from_core */
#else
		...
		printf("task_threads count = %d (after thread_start())\n", 
		       count);
		if(task_resume(target_task) == KERN_SUCCESS)
		     printf("task_resume worked\n");
		else
		     printf("task_resume failed\n");
		for(i = 0; i < count; i++){
		    k = thread_resume(threads[i]);
		    if(k == KERN_SUCCESS)
			 printf("thread_resume thread %d worked\n", i);
		    else
			 printf("thread_resume thread %d failed (%s)\n", i,
				mach_error_string(k));
#ifdef __i386__
			...
			printf("thread_get_state INTEGER failed\n");
