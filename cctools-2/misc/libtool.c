...
void)
{
	if(cmd_flags.ranlib)
	    ...
	    fprintf(stderr, "Usage: %s -dynamic [-] file [...] "
		    ...
		    "[-seg1addr 0x#] [-all_load] [-noall_load]\n", progname);
	}
	exit(EXIT_FAILURE);
}
...
struct ofile *ofile)
{
	...
	if(ofile->mh != NULL)
	    ...
	    /*
	     * If this ofile member is using extend format #1 for it's name the
	     * remove the size of the name from the member_size.
	     */
	    if(ofile->member_name != ofile->member_ar_hdr->ar_name){
		size = round(ofile->member_size - ofile->member_name_size,
			     sizeof(long));
	    }
	    else{
	...
	if(ofile->mh != NULL){
	    ...
	    /*
	     * If this ofile member is using extend format #1 for it's name the
	     * remove the size of the name in included in the the member_size
	     * and the member_addr.
	     */
	    if(ofile->member_name != ofile->member_ar_hdr->ar_name){
...
char *output)
{
	...
	for(i = 0; i < narchs; i++){
		...
		if(narchs > 1)
		    error("warning library: %s for architecture: %s the table "
			  "of contents is empty (no object file members in the "
			  "library)", output, arch->arch_flag.name);
		else
		    error("warning for library: %s the table of contents is "
