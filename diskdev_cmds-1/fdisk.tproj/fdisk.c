...
usage()
{
	...
	fprintf(stderr, "raw-device is /dev/rsd0h for SCSI, /dev/rhd0h for IDE\n");
...
main(int argc, char *argv[])
{
	...
	else if ((strncmp(cp, "/dev/rhd", 8) &&
		strncmp(cp, "/dev/rsd", 8)) || (cp[9] != 'h'))
	{
		usage();
	}
...
fbegin:
	...
	{
		bomb("fdisk: unable to get disk info\n");
	}
...
showDiskInfo()
{
	...
	for (i=0; i<4; i++)
	{
		fp = (struct fdisk_part *)bootsector.parts[i];
