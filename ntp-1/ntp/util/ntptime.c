...
-t timeconstant	log2 of PLL time constant (0 .. %d)\n",
	...
	if (status < 0)
		...
		printf("  offset %ld us, frequency %.3f ppm, interval %d s,\n",
		       (long)ntx.offset, ftemp, 1 << ntx.shift);
		printf("  maximum error %lu us, estimated error %lu us,\n",
		       (u_long)ntx.maxerror, (u_long)ntx.esterror);
		ftemp = ntx.tolerance;
		ftemp /= scaled_usec(1);
		printf("  status %s,\n", sprintb(ntx.status, TIMEX_STA_BITS));
		printf("  time constant %lu, precision %lu us, tolerance %.0f ppm,\n",
		       (u_long)ntx.constant, (u_long)ntx.precision, ftemp);
		if (ntx.shift == 0)
		    exit (0);
		ftemp = ntx.ppsfreq;
		ftemp /= scaled_usec(1);
		gtemp = ntx.stabil;
		gtemp /= scaled_usec(1);
		printf("  pps frequency %.3f ppm, stability %.3f ppm, jitter %lu us,\n",
		     ftemp, gtemp, (u_long)ntx.jitter);
