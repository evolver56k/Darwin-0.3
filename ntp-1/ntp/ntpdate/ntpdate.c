...
#ifdef USE_NETINFO
static void addnetinfoservers P((void));
#endif
	...
	)
{
	 ...
	 */
#ifdef USE_NETINFO
	while ((c = ntp_getopt(argc, argv, "a:bBde:k:No:p:qr:st:uv")) != EOF)
#else
...
#ifdef USE_NETINFO
	sys_maxservers = argc - ntp_optind + netinfo_servers;
#else
	...
	if (errflg || sys_maxservers == 0) {
		(void) fprintf(stderr,
#ifdef USE_NETINFO
				   "usage: %s [-bBdNqsv] [-a key#] [-e delay] [-k file] [-p samples] [-o version#] [-r rate] [-t timeo] server ...\n",
#else
...
# endif /* LOG_DAEMON */
	 ...
	 */
#ifdef USE_NETINFO
	if (use_netinfo) addnetinfoservers();
#endif
...
addnetinfoservers(void)
{
	...
	if (whatHappened != NI_OK) {
		msyslog(LOG_ERR,
		    "Problem obtaining NTP initialization data, aborting. Unable to set time\n");
	...
	if ((whatHappened == NI_OK) && currentProp.ni_namelist_len && 
	    ...
	    if (sscanf(currentProp.ni_namelist_val[0], "%s", 
		       hostBuffer) != 1)
		msyslog(LOG_ERR, "init_ntp: initialization error, server\n");
	ni_namelist_free(&currentProp);
	ni_free(currentDomain);
