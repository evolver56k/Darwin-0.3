...
static	void	permc	(u_char *, u_int32 *);
static	void	permd	(u_char *, u_int32 *);
...
u_int32 masks[4] = { 0x40000000, 0x400000, 0x4000, 0x40 };
...
doit(void)
{
	...
	printf("static u_int32 PC2_C[4][64] = {");
	for (i = 0; i < 4; i++) {
	...
	printf("static u_int32 PC2_D[4][64] = {");
	for (i = 0; i < 4; i++) {
		for (comb = 0; comb < 64; comb++) {
			if (comb & 0x20)
			    bits[dbits[i][0]] = 1;
