...
#ifdef USE_NETINFO
static const char *ntp_options = "aAbc:dD:f:gk:l:mNnp:r:s:t:v:V:x";
#else
static const char *ntp_options = "aAbc:dD:f:gk:l:mnp:r:s:t:v:V:x";
#endif
...
#ifdef USE_NETINFO
/*
 * netinfo configuration stuff
 */

static int	using_config_file = 1;
static char	**server_list = 0;
static char	**peer_list = 0;
int		ntpversion = 0; 	/* Backwards compatibility */

static	void	get_netinfo_config P((char **));
#endif
...
#ifdef USE_NETINFO
		(void) fprintf(stderr, "usage: %s [ -abdgmNnx ] [ -c config_file ] [ -e e_delay ]\n", progname);
#else
...
#ifdef USE_NETINFO
			case 'N':
			get_netinfo_config(&config_file);
			break;
#endif
...
#ifdef USE_NETINFO
	if (using_config_file && (fp = fopen(FindConfig(config_file), "r")) == NULL)
#else
		...
		switch(tok) {
			...
			if (!getnetnum(tokens[1], &peeraddr, 0)) {
				...
				if (((tok == CONFIG_SERVER) ||
					...
					!ISREFCLOCKADR(&peeraddr) &&
#endif
					IN_CLASSD(peeraddr.sin_addr.s_addr)) {
					msyslog(LOG_ERR,
						"attempt to configure invalid address %s",
						ntoa(&peeraddr));
					break;
...
#ifdef USE_NETINFO
			/*
			 * If sysadmin sets backward compatibility property
			 * in NetInfo, we use its value as the NTP protocol
			 * version number.
			 */
			if ( ntpversion >= NTP_OLDVERSION &&
			     ntpversion <= NTP_VERSION )
				peerversion = ntpversion;
			else
#endif
...
#ifdef USE_NETINFO
	static int finished_with_servers = 0;
	static int finished_with_peers = 0;
	static int index = 0;

	if (!using_config_file) {

		while (!finished_with_servers) {
			if (server_list && server_list[index]) {
				*ntokens = 2;
				tokenlist[0] = "server";	/* just in case */
				tokenlist[1] = server_list[index];
				msyslog(LOG_INFO, "using host %s as a server", server_list[index]);
				index++;
				return CONFIG_SERVER;
			} else {
				finished_with_servers++;
				index = 0;
			}
		}
		while (!finished_with_peers) {
			if (peer_list && peer_list[index]) {
				*ntokens = 2;
				tokenlist[0] = "peer";		/* just in case */
				tokenlist[1] = peer_list[index];
				msyslog(LOG_INFO, "using host %s as a peer", peer_list[index]);
				index++;
				return CONFIG_PEER;
			} else {
				finished_with_peers++;
...
/* 
 *	this function reads the configuration info out of netinfo, it looks for
 *	three properties configfile, host, server.  if it finds configfile it 
 *	will tell xntpd to use that for all configuration and ignore the rest
 *	of the properties in netinfo.  this could be useful for setting up servers
 *	in the configuration.
 *
 *	the other method uses the host and server properties.  these properties
 *	should behave the same as in openstep 4.2, see comments below to better
 *	understand how they work.
 */

void
get_netinfo_config(char **config_file)
{
	...
	/* find what level has /locations/ntp */
	...
	if (whatHappened != NI_OK) {
		msyslog(LOG_ERR,"Problem obtaining NTP data from /locations/ntp.\n");
		msyslog(LOG_ERR, "     Problem was: %s", ni_error(whatHappened));
		exit(1);
	}
	NI_INIT(&currentProp);

	/* check for "configfile", if found use it and return */

	using_config_file = 0;

	whatHappened = ni_lookupprop(currentDomain, &locDir, "configfile", &currentProp);
	if ((whatHappened == NI_OK) && currentProp.ni_namelist_len && 
		...
		if (sscanf(currentProp.ni_namelist_val[0], "%s", name) != 1) {
			msyslog(LOG_ERR, "get_netinfo_config: initialization error parsing configfile");
			exit(1);
		} else {
			if (*config_file = malloc(strlen(name)+1))
	...
	/* If config_file is set we ignore all other values */
	if (using_config_file) {
		msyslog(LOG_INFO, "ntp configuration file set to %s", *config_file);
		return;
	}

	/* check for "host", if we are in the list, we are a peer to the list members, 
	   if we are not in list then it is a list of our servers */
	gethostname(myHostname, 256);
	myHostEntry = gethostbyname(myHostname);

	if (myHostEntry == NULL)
	{
		msyslog(LOG_ERR, "get_netinfo_config: gethostbyname returns NULL");
		exit(1);
	}

	whatHappened = ni_lookupprop(currentDomain, &locDir, "host", &currentProp);
	if (whatHappened == NI_OK) {
		index = ni_namelist_match(currentProp, myHostname);
		if (index != NI_INDEX_NULL) {
			...
			while (myHostEntry->h_aliases[i] != 0) {
				index = ni_namelist_match(currentProp, myHostEntry->h_aliases[i]); 
				if (index != NI_INDEX_NULL){
					skip = index;
					inHostList = 1;
					break;
		...
		list = malloc(sizeof(char **) * (currentProp.ni_namelist_len + 1));
		for (index = 0, i=0; index < currentProp.ni_namelist_len; index++) {
			if (inHostList && index == skip) continue;
			if (list[i]= malloc(strlen(currentProp.ni_namelist_val[index])+1)) {
				strcpy(list[i], currentProp.ni_namelist_val[index]);
				i++;
			}
		}
		list[i] = 0;
	...
	/* check for "server", if we are in the list, we are a peer to the list members,
	   if we not in the list, but we were in the "host" list, then this is a list
	   of our servers */

	whatHappened = ni_lookupprop(currentDomain, &locDir, "server", &currentProp);
	if (whatHappened == NI_OK) {
		index = ni_namelist_match(currentProp, myHostname);
		if (index != NI_INDEX_NULL) {
		    skip = index;
		...
		list = malloc(sizeof(char **) * (currentProp.ni_namelist_len + 1));
		for (index = 0, i=0; index < currentProp.ni_namelist_len; index++) {
			if (inServerList && index == skip) continue;
			if (list[i]= malloc(strlen(currentProp.ni_namelist_val[index])+1)) {
				strcpy(list[i], currentProp.ni_namelist_val[index]);
				i++;
			}
		...
		list[i] = 0;
		if (inServerList) {
			server_list = 0;	/* toss the server list (if we have one) */
			peer_list = list;
		} else if (inHostList || (!server_list && !peer_list)) {
			server_list = list;
		}
	...
	/* Check NetInfo for "version" property.  If set, this will
	 * have the following effects:
	 *
	 * (1) Use its value as the protocol version number for talking
	 *     to other servers, and
	 * (2) If protocol version 1 selected, accept MODE_PASSIVE replies
	 *     even if peer is configured with MODE_CLIENT (because
	 *     protocol 1 servers, including OPENSTEP4.x/NEXTSTEP3.x, seem
	 *     to always respond in symmetric passive mode to client
	 *     requests).
	 *
	 * Backwards compatibility is not enabled if a config file is used.
	 * Anyone using a config file is presumed to know how to config
	 * ntpd to do what they want.
	 */

	whatHappened = ni_lookupprop(currentDomain, &locDir,
				"version", &currentProp);
	if ((whatHappened == NI_OK) && currentProp.ni_namelist_len
	     && currentProp.ni_namelist_val[0]) {
		if (sscanf(currentProp.ni_namelist_val[0], "%s", name) != 1) {
			msyslog(LOG_ERR, "get_netinfo_config: error reading version property");
			exit(1);
		} else {
			ntpversion = atoi(name);
			if ( (ntpversion < NTP_OLDVERSION) ||
			     (ntpversion > NTP_VERSION) ) {
				msyslog(LOG_ERR, "get_netinfo_config: version property (%d) is out of bounds: must be between %d and %d inclusive", ntpversion, NTP_OLDVERSION, NTP_VERSION);
				exit(1);
			}
		}
	}
