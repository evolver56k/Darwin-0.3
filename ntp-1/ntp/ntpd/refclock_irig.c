 ...
 *
 * o A frame synchronization error has occured. This usually means wrong
 *   IRIG signal format or the IRIG signal source has lost
 *   synchronization (signature control).
 *
 * o A data decoding error has occured. This usually means wrong IRIG
 *   signal format.
 *
 ...
 *
 * o An audio codec error (overrun) occured. This usually means
 ...
 */
#define	PRECISION	(-16)	/* precision assumed (about 10 us) */
	...
	)
{
	...
	if (bit == BIT1) {
		 ...
		 * clock, but only if no errors have occured.
		 */
		up->bitcnt = 1;
		up->fieldcnt = 0;
		up->lastbit = 0;
	...
	if (up->bitcnt == 0) {
		...
		if (up->fieldcnt == 0) {
			 ...
			 */
			up->xptr = 2 * FIELD;
			sscanf((char *)up->timecode, "%6s%2d%c%2s%3d%2d%2d%2d",
