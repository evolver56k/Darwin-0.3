...
/**
 * If you are building a fat binary or cross-compiling, then
 * setting HOSTTYPE and MACHTYPE from the autoconf host doesn't
 * work.  Use __ARCHITECTURE__ if provided by the compiler.
 * It would be better if Makefile.in passed in -DCONF_VENDOR
 * than -DMACHTYPE, so the #ifdef __VENDOR__ business can go
 * away.
 **/
...
# if defined(__ppc__)
#  define HOSTTYPE "powerpc" /* ppc == powerpc */
# else
#  define HOSTTYPE __ARCHITECTURE__
# endif
# if defined(__APPLE__)
#  define MACHTYPE HOSTTYPE "-apple-" OSTYPE
# elif defined(__NeXT__)
#  define MACHTYPE HOSTTYPE "-next-" OSTYPE
# else
#  define MACHTYPE HOSTTYPE "-unknown-" OSTYPE
# endif
#else
